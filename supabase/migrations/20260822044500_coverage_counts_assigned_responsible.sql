...
with month_bounds as (
  ...
  select c.id as client_id,
         count(v.id) filter (where v.ended_at is not null and v.ended_at >= mb.month_start and v.ended_at < mb.month_end) as visits_this_month,
         max(v.ended_at) filter (where v.ended_at is not null) as last_visit_at
  from public.clients c
  cross join month_bounds mb
  left join public.visits v on v.client_id=c.id and v.employee_id=c.vendor_employee_id
  group by c.id
), call_stats as (
  select c.id as client_id,
         count(cl.id) filter (where cl.occurred_at >= mb.month_start and cl.occurred_at < mb.month_end) as calls_this_month,
         max(cl.occurred_at) as last_call_at
  from public.clients c
  cross join month_bounds mb
  left join public.calls cl on cl.client_id=c.id and cl.employee_id=c.manager_employee_id
  group by c.id
)
