...
with planned as(
 ...
 select employee_id,session_date as day,count(*)::int routes_started,count(*) filter(where status='FINALIZADA')::int routes_completed,min(started_at) route_first_at,max(coalesce(ended_at,now())) route_last_at,sum(extract(epoch from(coalesce(ended_at,now())-started_at)))::bigint route_window_seconds
 from public.route_sessions group by employee_id,session_date
), visited as(
 select employee_id,(started_at at time zone 'America/Santo_Domingo')::date as day,
 count(*) filter(where ended_at is not null)::int visited_clients,
 count(*) filter(where ended_at is not null and coalesce(received,false))::int received_clients,
 count(*) filter(where ended_at is not null and lower(coalesce(purchase_result,'')) in('compro','compró','compra','si','sí','yes'))::int visit_purchase_clients,
 coalesce(sum(purchase_amount) filter(where ended_at is not null and lower(coalesce(purchase_result,'')) in('compro','compró','compra','si','sí','yes')),0)::numeric(14,2) visit_sales_amount,
 coalesce(sum(extract(epoch from(coalesce(ended_at,now())-started_at))),0)::bigint visit_seconds,min(started_at) visit_first_at,max(coalesce(ended_at,now())) visit_last_at
 ...
 coalesce(sum(extract(epoch from(coalesce(ended_at,now())-started_at))),0)::bigint showroom_seconds,min(started_at) showroom_first_at,max(coalesce(ended_at,now())) showroom_last_at
 ...
 min(started_at) filter(where status<>'CANCELADA') incident_first_at,max(coalesce(ended_at,now())) filter(where status<>'CANCELADA') incident_last_at
