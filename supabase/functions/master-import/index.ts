...
function baseKey(v: string | null) { return normalizeKey(v).split('-')[0].trim() }
...
type MappingRule = { mapping_status: string; employee_id: string | null }
function resolveEmployee(rules: Map<string, MappingRule>, aliases: Map<string, string>, key: string) {
  const rule = rules.get(key)
  if (rule) return rule.mapping_status === 'EMPLOYEE' ? rule.employee_id : null
...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    for (const e of employees || []) {
      const full = normalizeKey(e.full_name)
      const first = full.split(' ')[0]
      if (e.employee_type === 'Vendedor') { vendorAliases.set(full, e.id); if (first) vendorAliases.set(first, e.id) }
      if (e.employee_type === 'Gestor') { managerAliases.set(full, e.id); if (first) managerAliases.set(first, e.id) }
    }

    const { data: mappingRows, error: mappingError } = await admin.from('portfolio_mappings').select('portfolio_type,source_key,mapping_status,employee_id')
    if (mappingError) throw mappingError
    const vendorRules = new Map<string, MappingRule>()
    const managerRules = new Map<string, MappingRule>()
    for (const rule of mappingRows || []) {
      const item = { mapping_status: rule.mapping_status, employee_id: rule.employee_id }
      if (rule.portfolio_type === 'V') vendorRules.set(normalizeKey(rule.source_key), item)
      if (rule.portfolio_type === 'G') managerRules.set(normalizeKey(rule.source_key), item)
    }
    ...
    rows.forEach((raw: Record<string, unknown>, index: number) => {
      const rowNo = index + 2
      const companyCode = text(raw['Empresa'])
      const companyId = companyMap.get(normalizeKey(companyCode)) || null
      const codempr = text(raw['codempr'])
      const legalName = text(raw['RazonSocial']) || text(raw['Nombre'])
      const geo = coordinates(raw['longitud'], raw['latitud'])
      const vCartera = text(raw['V-CARTERA'])
      const gCartera = text(raw['G-CARTERA'])
      const vendorBase = baseKey(vCartera)
      const managerBase = baseKey(gCartera)
      if (!codempr) errors.push({ row: rowNo, code: null, message: 'codempr vacío' })
      if (!legalName) errors.push({ row: rowNo, code: codempr, message: 'Razón social vacía' })
      if (!companyId) errors.push({ row: rowNo, code: codempr, message: `Empresa no reconocida: ${companyCode ?? '(vacía)'}` })
      if (!codempr || !legalName || !companyId) return

      normalized.push({
        company_id: companyId, company_code: companyCode, map_code: text(raw['Cod Map']), source_id: intVal(raw['id']), codempr, cod_empresa: text(raw['cod empresa']),
        v_cartera: vCartera, g_cartera: gCartera,
        vendor_employee_id: resolveEmployee(vendorRules, vendorAliases, vendorBase),
        manager_employee_id: resolveEmployee(managerRules, managerAliases, managerBase),
        region: text(raw['REGION']), province: text(raw['PROVINCIA']), municipality: text(raw['MUNICIPIO']), client_type: text(raw['Tipo']), legal_name: legalName,
        contact_name: text(raw['Contacto']), address1: text(raw['Direccion1']), display_name: text(raw['Nombre']), sector_id: text(raw['sector_id']), phone1: text(raw['Telefono1']), phone2: text(raw['Telefono2']), mobile: text(raw['Celular']), email: text(raw['Email']),
        entry_date: excelDate(raw['fecha_ingreso']), entry_year: intVal(raw['Year Ingreso']), type_id: text(raw['tipo_id']), category_id: text(raw['categoria_id']), account_type_id: text(raw['tipo_cuenta_id']), zone_id: text(raw['zona_id']), vendor_source_id: text(raw['vendedor_id']), collector_source_id: text(raw['cobrador_id']), municipality_source_id: text(raw['municipo_id']), manager_service_source_id: text(raw['gestor_servicio_id']),
        longitude: geo.longitude, latitude: geo.latitude, npg_code: text(raw['codigo_npg']), active_status: text(raw['Activo']), segment_id: text(raw['segmento_id']), last_receipt_date: excelDate(raw['fecha_ultimo_recibo']), last_receipt_amount: numVal(raw['monto_ultimo_recibo']), identification_type: text(raw['tipo_identificacion']), identification: text(raw['identificacion']), last_invoice_date: excelDate(raw['fecha_ultima_factura']), last_invoice_year: intVal(raw['Year Ult Fact']), last_invoice_amount: numVal(raw['monto_ultima_factura']), payment_type: text(raw['TipoPago']), municipality_code: text(raw['Id Mun']), sector_code: text(raw['Id Sector']), client_type_code: text(raw['Id Tipo Cli']), credit_days: intVal(raw['Dias Credito']),
        source_data: raw,
        data_quality: { has_valid_georef: geo.valid, import_source: sourceName, import_sheet: 'cartera', initial_master: true },
      })
    })
    ...
    console.error('master-import failed', error instanceof Error ? error.message : 'unknown')
    return json({ error: 'No fue posible procesar el maestro' }, 500)
  }
})
