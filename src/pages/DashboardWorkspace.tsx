...
export function DashboardWorkspace(){
  ...
  const load=async()=>{
    ...
    const fields='route_plan_id,route_session_id,employee_id,route_date,derived_status,route_mode,planned_clients,visited_clients,total_completed_visits,resolved_clients,coverage_pct'
    let q=supabase.from('executive_route_journeys_v4').select(fields).eq('route_date',today())
    if(!executive)q=q.eq('employee_id',employee.id)
    const current=await q
    let staleQ=supabase.from('executive_route_journeys_v4').select(fields).eq('derived_status','PENDIENTE_CIERRE')
