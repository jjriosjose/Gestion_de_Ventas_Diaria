...
import { Camera, Check, MapPinCheck, X } from 'lucide-react'
...
export function Visits() {
  ...
  return <div className="page-stack">
    <div className="page-head"><div><span className="eyebrow">GESTIÓN DE CALLE</span><h2>Visitas</h2><p>La llegada inicia la visita y la salida la finaliza para medir permanencia real en el cliente.</p></div><div className="button-row"><button className="secondary" onClick={() => void exportXlsx('Gestion_Visitas', report)}>Excel</button><button className="secondary" onClick={() => exportPdf('Gestión de Visitas', report)}>PDF</button></div></div>
...
function FinishVisit({ row, onClose, onSaved }: { row: any; onClose: () => void; onSaved: () => void }) {
    ...
    try {
      ...
      if (nextAction === 'SHOWROOM') {
        ...
        if (managerId) {
          ...
          alert('La visita se guardó, pero este cliente no tiene V-Gestor homologado. La solicitud de showroom quedó sin generar.')
        }
      }
      onSaved()
    } catch (e) {
  ...
  return <div className="modal-wrap"><button className="modal-backdrop" onClick={onClose}/><div className="modal large"><div className="modal-head"><div><span className="eyebrow">SALIDA DEL CLIENTE</span><h3>{row.clients?.legal_name || 'Cliente'}</h3><p>Al guardar se registra hora y GPS de salida.</p></div><button className="icon-btn" onClick={onClose}><X/></button></div><div className="form-grid"><label>¿Lo recibieron?<select value={received} onChange={e => setReceived(e.target.value)}><option value="si">Sí</option><option value="no">No</option></select></label><label>Quién lo atendió<input value={contact} onChange={e => setContact(e.target.value)} placeholder="Nombre / cargo"/></label><label>Resultado comercial<select value={purchase} onChange={e => setPurchase(e.target.value)}><option value="COMPRO">Compró</option><option value="NO_COMPRO">No compró</option><option value="PENDIENTE">Pendiente</option></select></label><label>Resultado visita<select value={result} onChange={e => setResult(e.target.value)}><option value="RECIBIDO">Recibido</option><option value="NO_RECIBIDO">No recibido</option><option value="CERRADO">Cerrado</option><option value="NO_LOCALIZADO">No localizado</option><option value="REPROGRAMAR">Reprogramar</option></select></label><label className="span-2">Motivo no compra<input value={reason} onChange={e => setReason(e.target.value)}/></label><label className="span-2">Comentario mercancía Karaka<textarea value={karaka} onChange={e => setKaraka(e.target.value)}/></label><label className="span-2">Comentario competencia<textarea value={competition} onChange={e => setCompetition(e.target.value)}/></label><label>Próxima acción<select value={nextAction} onChange={e => setNextAction(e.target.value)}><option value="SIN_SEGUIMIENTO">Sin seguimiento</option><option value="VOLVER_VISITAR">Volver a visitar</option><option value="LLAMAR">Llamar</option><option value="SHOWROOM">Interesado en showroom</option><option value="ENVIAR_INFO">Enviar información</option><option value="OTRO">Otro</option></select></label><label>Fecha seguimiento<input type="date" value={followUp} onChange={e => setFollowUp(e.target.value)}/></label>{nextAction === 'SHOWROOM' && <><label>Fecha/hora tentativa showroom<input type="datetime-local" value={showroomDate} onChange={e => setShowroomDate(e.target.value)}/></label><label>Asignación<input disabled value={row.clients?.manager_employee_id ? 'Automática al V-Gestor' : 'SIN GESTOR HOMOLOGADO'}/></label></>}<label className="span-2">Fotos / evidencia<div className="field"><Camera size={18}/><input type="file" accept="image/*" capture="environment" multiple onChange={e => setFiles(Array.from(e.target.files || []))}/></div><small>{files.length ? `${files.length} foto(s) seleccionada(s)` : 'Puedes tomar o adjuntar fotos de la gestión.'}</small></label><label className="span-2">Observación<textarea value={notes} onChange={e => setNotes(e.target.value)}/></label></div><div className="modal-actions"><button className="secondary" onClick={onClose}>Cancelar</button><button className="primary" disabled={busy} onClick={() => void save()}>{busy ? 'Guardando salida...' : 'Finalizar visita y salir'}</button></div></div></div>
}
