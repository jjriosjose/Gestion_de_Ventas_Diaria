...
export function Routes() {
  ...
  const startVisit = async (stop: any) => {
    if (!session || !mine) return alert('Debes iniciar la ruta primero')
    setBusy(true)
    try {
      const position = await currentPosition()
      ...
      alert('Visita iniciada. Finalízala desde el módulo Visitas.')
    } catch (error) {
      alert(error instanceof Error ? error.message : 'Error al iniciar la visita')
    } finally {
      setBusy(false)
      void load()
    }
