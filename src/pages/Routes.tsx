...
import { RouteCaptureModal } from '../components/RouteCaptureModal'
...
export function Routes(){
  ...
  const [captureRows,setCaptureRows]=useState<any[]>([])
  ...
  const load=async()=>{
    ...
    if(selected&&!scoped.some(p=>p.id===selected.id)){setSelected(null);setStops([]);setSession(null);setSessionVisits([]);setIncidents([]);setCaptureRows([])}
    ...
    if(selected&&scoped.some(p=>p.id===selected.id)){
      ...
      if(latestSession){
        const [{data:incidentRows},{data:visitRows},{data:captureData,error:captureError}]=await Promise.all([
          ...
          supabase.from('capture_interactions').select('id,route_session_id,route_plan_id,employee_id,status,result_code,subject_name,contact_name,phone,client_type,business_interest,started_at,ended_at,prospect_id,notes').eq('route_session_id',latestSession.id).order('started_at',{ascending:true})
        ])
        if(captureError)throw captureError
        setIncidents(incidentRows||[])
        setSessionVisits(visitRows||[])
        setCaptureRows(captureData||[])
      }else{setIncidents([]);setSessionVisits([]);setCaptureRows([])}
    }else if(!selected){setStops([]);setSession(null);setSessionVisits([]);setIncidents([]);setCaptureRows([])}
  ...
  const activeCapture=captureRows.find(row=>row.status==='ACTIVA'&&!row.ended_at)||null
  const completedCaptures=captureRows.filter(row=>row.status==='FINALIZADA').length
  const successfulCaptures=captureRows.filter(row=>row.status==='FINALIZADA'&&row.result_code==='CAPTADO').length
  ...
  const startVisit=async(stop:any)=>{
    ...
    if(activeCapture)return alert(`Tienes una captación en curso en ${activeCapture.subject_name||'un negocio'}. Finalízala antes de iniciar una visita.`)
    setBusy(true)
    try{
      const p=await currentPosition()
  ...
  return <div className="page-stack">
    ...
    <div className="route-workspace">
      ...
      <div className="panel route-detail route-detail-compact">{!selected?<div className="empty-state"><MapPin/><b>Selecciona una ruta para ver mapa y secuencia</b></div>:<>
        ...
        {mine&&(activeSession||selected.status==='PLANIFICADA')&&<div className="route-actions">{activeSession?<><span className="live-pill"><i/> {selected.route_mode==='LIBRE'?'Jornada libre':'Ruta'} activa desde {new Date(activeSession.started_at).toLocaleTimeString('es-DO',{hour:'2-digit',minute:'2-digit'})}</span>{!openVisit&&!activeCapture&&<button className="primary compact" disabled={!!activeIncident} title={activeIncident?'Finaliza la eventualidad activa':selected.route_mode==='LIBRE'?'Registrar llegada a un cliente':'Registrar cliente fuera del plan'} onClick={()=>setAdditionalOpen(true)}><Plus size={16}/>{selected.route_mode==='LIBRE'?'Visitar cliente':'Visita adicional'}</button>}{!openVisit&&!activeIncident&&<button className={activeCapture?'primary compact':'secondary compact'} onClick={()=>setCaptureOpen(true)}><Target size={16}/>{activeCapture?'Finalizar captación':'Captar prospecto'}</button>}{openVisit&&<button className="primary compact" onClick={()=>navigate('/visitas')}><CheckCircle2 size={16}/> Finalizar visita actual</button>}<button className="danger" disabled={busy||!!openVisit||!!activeIncident||!!activeCapture} title={activeCapture?'Finaliza la captación antes de cerrar':activeIncident?'Finaliza la eventualidad antes de cerrar':''} onClick={()=>setClosureOpen(true)}><Square size={18}/> Cerrar jornada</button></>:<button className="primary" disabled={busy||selected.route_date!==today()} title={selected.route_date!==today()?`Disponible el ${selected.route_date}`:''} onClick={()=>void start()}><Play size={18}/> Iniciar ruta / salida</button>}</div>}
        ...
        {session&&<div className="route-incident-bar"><AlertTriangle size={18}/>{activeIncident?<><span className="incident-live"><i/> Eventualidad activa · {activeIncident.incident_type}</span><span>{durationLabel(activeIncident.started_at)}</span>{mine&&activeSession&&<button className="primary compact" disabled={busy} onClick={()=>void finishIncident()}>Finalizar eventualidad</button>}</>:<><span>Eventualidades de jornada: {incidents.length}</span>{mine&&activeSession&&<button className="secondary compact" disabled={!!activeCapture} title={activeCapture?'Finaliza la captación antes de registrar una eventualidad':''} onClick={()=>setIncidentOpen(true)}><Plus size={15}/> Registrar eventualidad</button>}</>}</div>}
        ...
        {(captureRows.length>0||activeSession)&&!isManager&&<div className="route-capture-list"><div className="route-capture-list-head"><div><b>Captaciones de esta jornada</b><span>Actividad comercial medida aparte de las visitas · {completedCaptures} finalizada(s) · {successfulCaptures} prospecto(s)</span></div><strong>{captureRows.length}</strong></div>{captureRows.length?captureRows.map(row=><div className="route-capture-row" key={row.id}><Target size={16}/><div className="main"><b>{row.subject_name}</b><span>{new Date(row.started_at).toLocaleTimeString('es-DO',{hour:'2-digit',minute:'2-digit'})}{row.ended_at?` → ${new Date(row.ended_at).toLocaleTimeString('es-DO',{hour:'2-digit',minute:'2-digit'})}`:' · en curso'} · {durationLabel(row.started_at,row.ended_at)}</span><small>{row.status==='ACTIVA'?'Captación en curso':row.status==='CANCELADA'?'Cancelada':String(row.result_code||'Finalizada').replaceAll('_',' ')}</small></div><span className={`state ${row.status==='ACTIVA'?'active':''}`}>{row.status==='ACTIVA'?'ACTIVA':row.result_code==='CAPTADO'?'PROSPECTO':'FINALIZADA'}</span></div>):<div className="empty-state"><Target/><b>Sin captaciones en esta jornada</b><span>Usa “Captar prospecto” cuando encuentres una oportunidad comercial durante la ruta.</span></div>}</div>}
    ...
    {captureOpen&&activeSession&&<RouteCaptureModal sessionId={activeSession.id} activeInteraction={activeCapture} onClose={()=>setCaptureOpen(false)} onChanged={async()=>{await load()}}/>}
