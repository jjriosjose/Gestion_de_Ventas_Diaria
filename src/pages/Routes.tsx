...
export function Routes(){
  ...
  const load=async()=>{
    ...
    if(employee?.id){
      const{data:openRows}=await supabase.from('visits').select('id,route_session_id,route_stop_id,client_id,started_at,clients(legal_name)').eq('employee_id',employee.id).is('ended_at',null).order('started_at').limit(1)
    ...
    if(selected&&scoped.some(p=>p.id===selected.id)){
      ...
      if(latestSession){
        const{data:incidentRows}=await supabase.from('operational_incidents').select('*').eq('route_session_id',latestSession.id).neq('status','CANCELADA').order('started_at',{ascending:false})
  ...
  const removePlan=async()=>{
    ...
    setBusy(false)
    if(error)return alert(error.message)
    setSelected(null);setStops([]);setSession(null);await load()
  }
  ...
  const report=displayStops.map(s=>({Orden:s.stop_order,Tipo:s.clients?.client_type||'',Cliente:s.clients?.legal_name||'',Codigo:s.clients?.codempr||'',Gestor:empName(s.clients?.manager_employee_id),Municipio:s.clients?.municipality||'',Prioridad:s.priority,Estado:s.status,Motivo:s.reason_not_visited||''}))
  ...
  return <div className="page-stack">
    ...
    <div className="route-workspace">
      <div className="panel route-list">{plans.length?plans.map(plan=><button key={plan.id} className={`route-card ${selected?.id===plan.id?'selected':''}`} onClick={()=>setSelected(plan)}><div><b>{empName(plan.employee_id)}</b><span>{plan.route_date} · {isManager?`${managerCounts[plan.id]||0} de mis clientes${clientType?` ${clientType}`:''}`:'Visitas'}</span></div><span className={`status ${plan.status?.toLowerCase()}`}>{plan.status}</span></button>):<div className="empty-state"><b>{isManager?'No tienes clientes programados en rutas para esta fecha/filtro.':'No hay rutas para esta fecha.'}</b></div>}</div>
      <div className="panel route-detail route-detail-compact">{!selected?<div className="empty-state"><MapPin/><b>Selecciona una ruta para ver mapa y secuencia</b></div>:<>
        <div className="route-detail-head"><div><b>{empName(selected.employee_id)}</b><span>{selected.route_date} · {selected.title}</span><span>{displayStops.length} paradas visibles{clientType?` ${clientType}`:''} · {mappedStops} con GPS</span></div><div className="button-row"><button className="secondary compact" onClick={()=>void exportXlsx(`Ruta_${selected.route_date}`,report)}>Excel</button><button className="secondary compact" onClick={()=>exportPdf(`Ruta ${selected.route_date}`,report)}>PDF</button>{admin&&selected.status==='PLANIFICADA'&&<button className="danger compact" disabled={busy} onClick={()=>void removePlan()}><Trash2 size={15}/> Eliminar prueba</button>}</div></div>
        <RoutePerformance stops={managerScopedStops} isManager={isManager} clientType={clientType}/>
        {mine&&<div className="route-actions">{!activeSession?<button className="primary" disabled={busy||selected.status==='FINALIZADA'||selected.route_date!==today()} title={selected.route_date!==today()?`Disponible el ${selected.route_date}`:''} onClick={()=>void start()}><Play size={18}/> Iniciar ruta / salida</button>:<><span className="live-pill"><i/> Ruta activa desde {new Date(activeSession.started_at).toLocaleTimeString('es-DO',{hour:'2-digit',minute:'2-digit'})}</span>{openVisit&&<button className="primary compact" onClick={()=>navigate('/visitas')}><CheckCircle2 size={16}/> Finalizar visita actual</button>}<button className="danger" disabled={busy||!!openVisit||!!activeIncident} title={activeIncident?'Finaliza la eventualidad antes de cerrar':''} onClick={()=>setClosureOpen(true)}><Square size={18}/> Cerrar jornada</button></>}</div>}
    ...
    {closureOpen&&activeSession&&selected&&<RouteClosureModal session={activeSession} stops={stops} seriousIncident={seriousIncident} onClose={()=>setClosureOpen(false)} onSaved={(result:any)=>{setClosureOpen(false);setSession({...activeSession,ended_at:result?.ended_at||new Date().toISOString(),status:'FINALIZADA',closure_mode:result?.closure_mode,closure_reason_code:result?.closure_reason_code,closure_reason_text:result?.closure_reason_text,closed_pending_count:result?.closed_pending_count||0});setSelected({...selected,status:'FINALIZADA'});void load()}}/>}
  </div>
}

function RoutePerformance({stops,isManager,clientType}:{stops:any[];isManager:boolean;clientType:ClientTypeFilterValue}){
  ...
  return <div style={{margin:'12px 0 16px',padding:'14px',border:'1px solid var(--border)',borderRadius:14,background:'var(--surface-soft)'}}>
    <div style={{display:'flex',justifyContent:'space-between',gap:12,alignItems:'center',flexWrap:'wrap',marginBottom:10}}><div><b>{isManager?'Rendimiento de mis clientes':'Rendimiento de la ruta'}</b><div style={{fontSize:12,opacity:.72}}>{isManager?'Calculado solo sobre los clientes asignados a tu gestión.':'Cobertura real sobre todas las paradas planificadas.'}{clientType?' El filtro de Tipo es solo visual y no altera estos indicadores.':''}</div></div><strong style={{fontSize:24}}>{coverage}% cobertura</strong></div>
    <div style={{height:10,borderRadius:999,overflow:'hidden',background:'var(--border)',marginBottom:12}}><div style={{height:'100%',width:`${Math.min(100,coverage)}%`,background:'var(--success)',transition:'width .2s ease'}}/></div>
    <div style={{display:'grid',gridTemplateColumns:'repeat(auto-fit,minmax(105px,1fr))',gap:8}}>
      ...
      <Metric label="Visitados" value={visited} detail={`${coverage}%`}/>
      <Metric label="En visita" value={inVisit}/>
      <Metric label="Pendientes" value={pending}/>
      <Metric label="No realizados" value={notVisited}/>
      <Metric label="Reprogramados" value={reprogrammed}/>
      <Metric label="Resueltos" value={resolved} detail={`${resolution}%`}/>
    </div>
  </div>
}
...
function RouteClosureModal({session,stops,seriousIncident,onClose,onSaved}:{session:any;stops:any[];seriousIncident:any;onClose:()=>void;onSaved:(result:any)=>void}){
  const reasons=[
    ['FIN_JORNADA','Fin de jornada / tiempo agotado'],
    ['TRAFICO_RETRASO','Tráfico o retrasos'],
    ['CAMBIO_PRIORIDAD','Cambio de prioridad autorizado'],
    ['EVENTUALIDAD','Eventualidad de jornada'],
    ['SUSPENSION_SUPERVISOR','Suspensión por supervisor'],
    ['REPROGRAMACION','Reprogramar todas las pendientes'],
    ['OTRO','Otro']
  ]
  const total=stops.length
  const visited=stops.filter(s=>s.status==='VISITADO').length
  ...
  const save=async()=>{
    ...
    try{
      let p:any=null
      try{p=await currentPosition()}catch{
        if(!window.confirm('No fue posible obtener el GPS de cierre. ¿Deseas cerrar la jornada sin ubicación final?'))return
      }
      const{data,error}=await supabase.rpc('finalize_route_session',{
        p_route_session_id:session.id,
        p_reason_code:pending>0?reason:null,
        p_notes:notes.trim()||null,
        p_end_latitude:p?.latitude??null,
        p_end_longitude:p?.longitude??null,
        p_end_accuracy_m:p?.accuracy??null
      })
      if(error)throw error
      onSaved(data)
  ...
  return <div className="modal-wrap"><button className="modal-backdrop" onClick={busy?undefined:onClose}/><div className="modal large route-closure-modal"><div className="modal-head"><div><span className="eyebrow">CIERRE DE JORNADA</span><h3>{pending>0?'Cerrar ruta con pendientes':'Finalizar jornada de ruta'}</h3><p>Al confirmar, la hora de jornada queda congelada y deja de seguir acumulando tiempo.</p></div><button className="icon-btn" disabled={busy} onClick={onClose}><X/></button></div>
    <div className="closure-summary-grid"><Metric label="Planificados" value={total}/><Metric label="Visitados" value={visited} detail={`${coverage}%`}/><Metric label="Resueltos" value={resolved} detail={`${resolution}%`}/><Metric label="Pendientes" value={pending}/></div>
    {pending>0?<><div className="closure-warning"><AlertTriangle size={19}/><div><b>Quedan {pending} parada(s) pendientes.</b><span>Debes indicar por qué termina la jornada. Esas paradas no se contarán como visitadas.</span></div></div><div className="form-grid"><label className="span-2">Motivo del cierre<select value={reason} onChange={e=>setReason(e.target.value)}>{reasons.map(([code,text])=><option value={code} key={code}>{text}</option>)}</select><small>{reason==='REPROGRAMACION'?'Las pendientes pasarán a REPROGRAMADO.':'Las pendientes pasarán a NO VISITADO con este motivo.'}</small></label><label className="span-2">Observación {reason==='OTRO'?'(obligatoria)':'(opcional)'}<textarea value={notes} onChange={e=>setNotes(e.target.value)} placeholder="Contexto adicional del cierre de jornada..."/></label></div></>:<div className="closure-ok"><CheckCircle2 size={20}/><div><b>No quedan paradas pendientes.</b><span>La ruta se cerrará normalmente y se registrará GPS/hora de salida.</span></div></div>}
    <div className="closure-readout"><span>Cobertura real <b>{coverage}%</b></span><span>Resolución actual <b>{resolution}%</b></span><span>Jornada acumulada <b>{durationLabel(session.started_at)}</b></span></div>
...
function RouteException({stop,onClose,onSaved}:{stop:any;onClose:()=>void;onSaved:()=>void}){
  ...
  const save=async()=>{
    setBusy(true)
    const reasonLabel=reasons.find(r=>r[0]===reason)?.[1]||reason
    const status=reason==='REPROGRAMADA'?'REPROGRAMADO':'NO_VISITADO'
    const{error}=await supabase.from('route_stops').update({status,exception_reason_code:reason,reason_not_visited:reasonLabel,notes:notes||null}).eq('id',stop.id)
    setBusy(false)
