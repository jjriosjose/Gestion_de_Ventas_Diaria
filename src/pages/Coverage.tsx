...
import { CheckCircle2, Clock3, MapPinCheck, Pencil, Play, Search, SlidersHorizontal, Square, X } from 'lucide-react'
...
export function Coverage() {
  ...
  const endFreeDay = async () => {
    ...
    try {
      ...
      const p = await currentPosition()
      const { error } = await supabase.from('route_sessions').update({ ended_at: new Date().toISOString(), end_latitude: p.latitude, end_longitude: p.longitude, end_accuracy_m: p.accuracy, status: 'FINALIZADA' }).eq('id', freeSession.id)
      if (error) throw error
      setFreeSession(null)
    } catch (error) {
  ...
  const startSpontaneousVisit = async (row: CoverageRow) => {
    ...
    try {
      ...
      const p = await currentPosition()
      const { error } = await supabase.from('visits').insert({ route_session_id: freeSession.id, client_id: row.client_id, employee_id: employee.id, visit_kind: 'ESPONTANEA', planned: false, started_at: new Date().toISOString(), start_latitude: p.latitude, start_longitude: p.longitude, start_accuracy_m: p.accuracy })
      if (error) throw error
      navigate('/visitas')
    } catch (error) {
...
function BulkPolicyModal({ mode, rows, onClose, onSaved }: { mode: 'VISITAS' | 'LLAMADAS'; rows: CoverageRow[]; onClose: () => void; onSaved: () => void }) {
