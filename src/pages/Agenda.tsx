...
export function Agenda() {
  ...
  const load = async () => {
    ...
    const [a, r, s, e] = await Promise.all([
      supabase.from('appointments').select('*,clients(codempr,legal_name,phone1,mobile,vendor_employee_id,manager_employee_id),prospects(prospect_code,legal_name,phone,mobile),manager:employees!appointments_assigned_manager_id_fkey(id,full_name,employee_type),requester:employees!appointments_requested_by_employee_id_fkey(id,full_name,employee_type)').order('created_at', { ascending: false }).limit(1500),
      supabase.from('reception_entries').select('*,clients(codempr,legal_name),prospects(prospect_code,legal_name),manager:employees!reception_entries_assigned_manager_id_fkey(full_name)').order('check_in_at', { ascending: false }).limit(1000),
      supabase.from('showroom_sessions').select('*').order('started_at', { ascending: false }).limit(1000),
  ...
  const scopeRows = useMemo(() => rows.filter(r => {
    ...
    return true
  }), [rows, managerFilter, sourceFilter, from, to, q])
  ...
  const canManage = (r: any) => admin || r.assigned_manager_id === employee?.id || r.employee_id === employee?.id
  ...
  return <div className="page-stack">
    ...
    <div className="panel planner-filter-panel">
      <div className="planner-filter-grid">
        <div className="search-field"><Search size={18}/><input value={q} onChange={e => setQ(e.target.value)} placeholder="Cliente, código, contacto, responsable..."/></div>
        <select value={managerFilter} onChange={e => setManagerFilter(e.target.value)}><option value="">Todos los gestores</option>{managers.map(m => <option value={m.id} key={m.id}>{m.full_name}</option>)}</select>
        <select value={sourceFilter} onChange={e => setSourceFilter(e.target.value)}><option value="">Todos los orígenes</option><option value="LLAMADA">Llamada</option><option value="VISITA">Visita vendedor</option><option value="MANUAL">Manual</option></select>
      ...
      <div className="planner-filter-actions"><div className="meta"><span>{visible.length} gestiones visibles</span><span>{scopeRows.length} en el período/cartera</span></div><button className="secondary" onClick={() => { setQ(''); setManagerFilter(''); setSourceFilter(''); setStatusFilter(''); setFrom(''); setTo('') }}>Limpiar filtros</button></div>
    </div>

    {loadError && <div className="panel"><b>Error cargando Agenda / Showroom</b><span>{loadError}</span></div>}

    <div className="panel">
      <div className="panel-head"><div><b>Embudo de citas</b><span>Las solicitudes aparecen desde que se generan. No cuentan como cita pactada hasta que el gestor las confirma.</span></div><span className="badge">{visible.length} registros</span></div>
      <div className="calendar-list">
        {!loadError && visible.length === 0 && <div className="empty-state"><b>No hay citas con los filtros actuales.</b></div>}
        {visible.map(r => {
          const reception = receptionByAppointment.get(r.id)
          const displayDate = r.appointment_at || r.requested_appointment_at
          return <div className="appointment-card" key={r.id}>
            <div className="date-box">{displayDate ? <><b>{new Date(displayDate).getDate()}</b><span>{new Date(displayDate).toLocaleDateString('es-DO', { month: 'short' })}</span></> : <><Clock3/><span>Por definir</span></>}</div>
            <div className="activity-main">
              <b>{r.clients?.legal_name || r.prospects?.legal_name || 'Cita'}</b>
              <span>{displayDate ? new Date(displayDate).toLocaleString('es-DO') : 'Sin fecha tentativa'} · Gestor: {r.manager?.full_name || 'Sin responsable'}</span>
              <small>{r.status.replaceAll('_', ' ')} · {r.appointment_type} · origen {r.source_type || 'MANUAL'}{r.requester?.full_name ? ` · solicitada por ${r.requester.full_name}` : ''}</small>
              {r.request_contact_name && <small>Contacto: {r.request_contact_name}{r.request_phone ? ` · ${r.request_phone}` : ''}</small>}
              {r.validation_notes && <small>Validación: {r.validation_notes}</small>}
              {reception && <small>Llegada: {new Date(reception.check_in_at).toLocaleString('es-DO')} · {reception.status.replaceAll('_',' ')}</small>}
            </div>
            {canManage(r) && <div className="row-actions">
              {r.status === 'PENDIENTE_VALIDACION' && <><button className="primary compact" onClick={() => setValidate(r)}><PhoneCall size={15}/> Validar</button><button className="secondary compact" onClick={() => void update(r.id, 'CONTACTANDO')}>No contesta</button></>}
              {r.status === 'CONTACTANDO' && <button className="primary compact" onClick={() => setValidate(r)}><PhoneCall size={15}/> Reintentar</button>}
              {['CONFIRMADA','PROGRAMADA','REPROGRAMADA'].includes(r.status) && !reception && <><button className="success-btn compact" onClick={() => void registerArrival(r)}><DoorOpen size={16}/> Llegó</button><button className="secondary compact" onClick={() => setValidate(r)}><RotateCcw size={15}/> Reprogramar</button><button className="secondary compact" onClick={() => void update(r.id, 'NO_ASISTIO')}><XCircle size={16}/> No asistió</button></>}
            </div>}
          </div>
        })}
      </div>
    ...
    <div className="panel">
      ...
      <div className="cards-list">
        {receptions.filter(r => !['SALIO','CANCELADO'].includes(r.status)).length === 0 && <div className="empty-state"><b>No hay personas actualmente dentro del showroom.</b></div>}
        {receptions.filter(r => !['SALIO','CANCELADO'].includes(r.status)).map(r => {
