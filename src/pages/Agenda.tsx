...
export function Agenda() {
  ...
  const startService = async (entry: any) => {
    ...
    if (!existing) {
      const responsible = entry.assigned_manager_id || employee.id
  ...
  return <div className="page-stack">
    ...
    <div className="panel planner-filter-panel"><div className="planner-filter-grid"><div className="search-field"><Search size={18}/><input value={q} onChange={e => setQ(e.target.value)} placeholder="Cliente, código, contacto, responsable..."/></div><select value={managerFilter} onChange={e => setManagerFilter(e.target.value)}><option value="">Todos los gestores</option>{managers.map(m => <option value={m.id} key={m.id}>{m.full_name}</option>)}</select><select value={sourceFilter} onChange={e => setSourceFilter(e.target.value)}><option value="">Todos los orígenes</option><option value="LLAMADA">Llamada</option><option value="VISITA">Visita vendedor</option><option value="MANUAL">Manual</option></select><select value={statusFilter} onChange={e => setStatusFilter(e.target.value)}><option value="">Todos los estados</option><option value="PENDIENTE_VALIDACION">Pendiente validación</option><option value="CONTACTANDO">Contactando</option><option value="CONFIRMADA">Confirmada</option><option value="REPROGRAMADA">Reprogramada</option><option value="NO_CONFIRMADA">No confirmada</option><option value="CANCELADA">Cancelada</option><option value="ASISTIO">Asistió</option><option value="NO_ASISTIO">No asistió</option><option value="FINALIZADA">Finalizada</option></select><input type="date" value={from} onChange={e => setFrom(e.target.value)}/><input type="date" value={to} onChange={e => setTo(e.target.value)}/></div><div className="planner-filter-actions"><div className="meta"><span>{visible.length} gestiones visibles</span><span>{scopeRows.length} en el período/cartera</span></div><button className="secondary" onClick={() => { setQ(''); setManagerFilter(''); setSourceFilter(''); setStatusFilter(''); setFrom(''); setTo('') }}>Limpiar filtros</button></div></div>
