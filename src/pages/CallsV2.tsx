...
import { CalendarClock,Check,ChevronDown,ChevronUp,History,PhoneCall,Search,ShoppingBag,Users,X } from 'lucide-react'
...
export function CallsV2(){
 ...
 const [calls,setCalls]=useState<any[]>([]),[visits,setVisits]=useState<any[]>([]),[appointments,setAppointments]=useState<any[]>([]),[showroomSessions,setShowroomSessions]=useState<any[]>([]),[clients,setClients]=useState<any[]>([]),[employees,setEmployees]=useState<Employee[]>([])
 ...
 const load=async()=>{
  ...
  try{
   const [callRes,visitRes,appointmentRes,showroomRes,staffRes,portfolio,areas,assessmentMap]=await Promise.all([
    ...
    supabase.from('appointments').select('id,client_id,status,appointment_at,requested_appointment_at,source_type,assigned_manager_id,requested_by_employee_id,created_from_call_id,created_at').order('created_at',{ascending:false}).limit(3000),
    supabase.from('showroom_sessions').select('id,appointment_id,client_id,manager_employee_id,started_at,ended_at,purchased,purchase_amount').not('started_at','is',null).order('started_at',{ascending:false}).limit(3000),
   ...
   const error=callRes.error||visitRes.error||appointmentRes.error||showroomRes.error||staffRes.error;if(error)throw error
   setCalls(callRes.data||[]);setVisits(visitRes.data||[]);setAppointments(appointmentRes.data||[]);setShowroomSessions(showroomRes.data||[]);setEmployees((staffRes.data||[]) as Employee[]);setClients(portfolio);setOfficialAreas(areas);setGeoAssessments(assessmentMap)
 ...
 const appointmentByCallId=useMemo(()=>{const m=new Map<string,any>();appointments.forEach(r=>{if(r.created_from_call_id)m.set(r.created_from_call_id,r)});return m},[appointments])
 ...
 const visibleShowroomVisits=useMemo(()=>showroomSessions.filter(session=>{
  if(session.client_id&&!territorialIds.has(session.client_id))return false
  if(callerFilter&&session.manager_employee_id!==callerFilter)return false
  if(from&&new Date(session.started_at)<new Date(`${from}T00:00:00`))return false
  if(to&&new Date(session.started_at)>new Date(`${to}T23:59:59`))return false
  return true
 }),[showroomSessions,territorialIds,callerFilter,from,to])
 const callSummaryByClient=useMemo(()=>{
  const map=new Map<string,{calls:number;purchases:number;purchaseAmount:number}>()
  visibleCalls.forEach(call=>{
   if(!call.client_id)return
   const current=map.get(call.client_id)||{calls:0,purchases:0,purchaseAmount:0}
   current.calls+=1
   if(call.result==='COMPRO'){current.purchases+=1;current.purchaseAmount+=Number(call.purchase_amount||0)}
   map.set(call.client_id,current)
  })
  return map
 },[visibleCalls])
 const showroomVisitsByClient=useMemo(()=>{
  const map=new Map<string,number>()
  visibleShowroomVisits.forEach(session=>{if(session.client_id)map.set(session.client_id,(map.get(session.client_id)||0)+1)})
  return map
 },[visibleShowroomVisits])
 ...
 const periodLabel=from||to?`${from?formatInputDate(from):'Inicio'} → ${to?formatInputDate(to):'Hoy'}`:'Todo el histórico visible'
 ...
 const allVisibleCallsExpanded=visibleCalls.length>0&&visibleCalls.slice(0,500).every(r=>expandedCallIds.has(r.id))
 const toggleCallDetail=(id:string)=>setExpandedCallIds(current=>{const next=new Set(current);if(next.has(id))next.delete(id);else next.add(id);return next})
 const toggleAllCallDetails=()=>{const rendered=visibleCalls.slice(0,500);setExpandedCallIds(current=>{const next=new Set(current);if(rendered.length&&rendered.every(r=>next.has(r.id)))rendered.forEach(r=>next.delete(r.id));else rendered.forEach(r=>next.add(r.id));return next})}
 ...
 return <div className="page-stack">
  ...
  {view==='PORTFOLIO'?<div className="crm-call-split" id="crm-cartera"><div className="panel crm-call-list-panel"><div className="panel-head"><div><b>Cartera priorizada</b><span>Selecciona un cliente y gestiona sin salir de esta pantalla.</span></div><span className="badge">{filteredPortfolio.length}</span></div><div className="cards-list crm-call-client-list">{loading?<div className="skeleton tall"/>:filteredPortfolio.slice(0,250).map(c=>{const call=lastCallByClient.get(c.id),appt=latestAppointmentByClient.get(c.id);return <div key={c.id} className={`activity-card crm-client-row ${selectedClientId===c.id?'selected':''}`}><div className="activity-main"><b>{c.legal_name}</b><span>{c.codempr} · {c.client_type||'SIN TIPO'} · {c.region||'—'} / {c.province||'—'} / {c.municipality||'—'}</span><small>V: {employeeName(c.vendor_employee_id)} · G: {employeeName(c.manager_employee_id)} · última llamada: {call?new Date(call.occurred_at).toLocaleString('es-DO'):'Nunca'}{appt?` · showroom ${String(appt.status).replaceAll('_',' ')}`:''}</small></div><button type="button" className="secondary compact" onClick={()=>manageClient(c.id)}>Gestionar</button></div>})}</div></div><div className="crm-call-workbench-slot">{selectedClient?<CallWorkbench client={selectedClient} employee={employee} lastCall={lastCallByClient.get(selectedClient.id)} lastVisit={lastVisitByClient.get(selectedClient.id)} lastAppointment={latestAppointmentByClient.get(selectedClient.id)} onSaved={load}/>:<div className="panel empty-state"><PhoneCall/><b>Selecciona un cliente para gestionar</b><span>El formulario aparecerá aquí inmediatamente y permanecerá visible mientras recorres la cartera.</span></div>}</div></div>:<section className="panel call-history-panel"><div className="panel-head call-history-head"><div><b>Historial de llamadas</b><span>{visibleCalls.length} registros visibles. Pulsa una gestión para consultar todo lo registrado por el gestor.</span></div><div className="button-row"><button type="button" className="secondary compact" disabled={!visibleCalls.length} onClick={toggleAllCallDetails}>{allVisibleCallsExpanded?<><ChevronUp size={15}/> Contraer todos</>:<><ChevronDown size={15}/> Expandir visibles</>}</button></div></div><div className="call-period-summary"><div className="call-period-summary-head"><div><span className="eyebrow">RESUMEN DEL PERÍODO</span><b>{periodLabel}</b></div><small>Se actualiza con territorio, cliente/búsqueda, gestor, ejecutor y fechas aplicables.</small></div><div className="kpi-grid compact-kpis call-period-kpis"><Kpi label="Llamadas" value={visibleCalls.length} sub="registros del período" Icon={PhoneCall}/><Kpi label="Compras registradas" value={purchaseRows.length} sub="resultado Compró" Icon={ShoppingBag}/><Kpi label="Monto compras" value={money(callSales)} sub="registrado en llamadas" Icon={ShoppingBag}/><Kpi label="Visitas Showroom" value={visibleShowroomVisits.length} sub="visitas reales iniciadas" Icon={CalendarClock}/></div></div><div className="cards-list call-history-list">{visibleCalls.length===0&&<div className="empty-state"><b>No hay llamadas con los filtros actuales.</b><span>Ajusta gestor, resultado, dirección o período.</span></div>}{visibleCalls.slice(0,500).map(r=>{const expanded=expandedCallIds.has(r.id);const callAppointment=appointmentByCallId.get(r.id);const purchaseAmount=Number(r.purchase_amount||0);return <article className={`call-history-card ${expanded?'expanded':''}`} key={r.id}><button type="button" className="call-history-summary" onClick={()=>toggleCallDetail(r.id)} aria-expanded={expanded}><div className="activity-icon"><PhoneCall/></div><div className="call-history-summary-main"><div className="call-history-title-row"><b>{r.clients?.legal_name||r.prospects?.legal_name||'Gestión telefónica'}</b><span className={`call-result-badge ${callResultTone(r.result)}`}>{labelResult(r.result)}</span></div><span>{r.caller?.full_name||'Usuario'} · {new Date(r.occurred_at).toLocaleString('es-DO')}</span><small>{r.call_direction==='ENTRANTE'?'Entrante':'Saliente'} · V: {employeeName(r.clients?.vendor_employee_id)} · G: {employeeName(r.clients?.manager_employee_id)}</small><div className="call-history-highlights">{purchaseAmount>0&&<strong>{money(purchaseAmount)}</strong>}{r.next_action&&<span>{labelNextAction(r.next_action)}{r.follow_up_date?` · ${new Date(r.follow_up_date+'T00:00:00').toLocaleDateString('es-DO')}`:''}</span>}{r.appointment_created&&<span>Showroom solicitado</span>}</div></div><div className="call-history-toggle">{expanded?<ChevronUp size={20}/>:<ChevronDown size={20}/>}<span>{expanded?'Ocultar':'Ver gestión'}</span></div></button>{expanded&&<div className="call-history-detail">{r.client_id&&(()=>{const clientPeriod=callSummaryByClient.get(r.client_id)||{calls:0,purchases:0,purchaseAmount:0};const showroomCount=showroomVisitsByClient.get(r.client_id)||0;return <div className="call-client-period-summary"><div className="call-client-period-title"><div><span>Resumen del cliente en el período</span><b>{periodLabel}</b></div><small>Contexto acumulado del cliente con los filtros actuales.</small></div><div className="call-client-period-grid"><div className="call-client-period-kpi"><span>Llamadas</span><strong>{clientPeriod.calls}</strong><small>veces llamado</small></div><div className="call-client-period-kpi"><span>Compras registradas</span><strong>{clientPeriod.purchases}</strong><small>{clientPeriod.purchaseAmount>0?money(clientPeriod.purchaseAmount):'Sin monto registrado'}</small></div><div className="call-client-period-kpi"><span>Visitas Showroom</span><strong>{showroomCount}</strong><small>visitas reales iniciadas</small></div></div></div>})()}<div className="call-detail-grid"><CallDetail label="Resultado de gestión" value={labelResult(r.result)||'—'} strong/><CallDetail label="Tipo de llamada" value={r.call_direction==='ENTRANTE'?'Entrante':r.call_direction==='SALIENTE'?'Saliente':'No registrada'}/><CallDetail label="Ejecutada por" value={r.caller?.full_name||'—'}/><CallDetail label="Fecha y hora" value={new Date(r.occurred_at).toLocaleString('es-DO')}/><CallDetail label="Persona contactada" value={r.contact_name||'No registrada'}/><CallDetail label="Teléfono utilizado" value={r.phone_used||'No registrado'}/><CallDetail label="Duración" value={callDurationLabel(r.duration_seconds)}/><CallDetail label="Venta registrada" value={purchaseAmount>0?money(purchaseAmount):'Sin monto registrado'} strong={purchaseAmount>0}/><CallDetail label="Próxima acción" value={labelNextAction(r.next_action)||'Sin próxima acción'}/><CallDetail label="Fecha de seguimiento" value={r.follow_up_date?new Date(r.follow_up_date+'T00:00:00').toLocaleDateString('es-DO'):'Sin fecha'}/><CallDetail label="Showroom" value={r.appointment_created?(callAppointment?`${showroomStatusLabel(callAppointment.status)}${callAppointment.appointment_at?` · ${new Date(callAppointment.appointment_at).toLocaleString('es-DO')}`:''}`:'Solicitud creada'):'No generado'}/><CallDetail label="Código cliente" value={r.clients?.codempr||'—'}/></div><div className="call-detail-note"><span>Observación de la gestión</span><p>{r.notes||'Sin observaciones registradas.'}</p></div></div>}</article>})}</div>{visibleCalls.length>500&&<div className="call-history-limit-note">Mostrando los 500 registros más recientes del filtro actual. Usa el período o el filtro por gestor para acotar la revisión.</div>}</section>}
...
function CallWorkbench({client,employee,lastCall,lastVisit,lastAppointment,onSaved}:{client:any;employee:any;lastCall:any;lastVisit:any;lastAppointment:any;onSaved:()=>Promise<void>|void}){
 ...
 const save=async()=>{
  ...
  setBusy(true)
  try{
   const payload:any={client_id:client.id,employee_id:employee.id,occurred_at:new Date().toISOString(),result,call_direction:direction,duration_seconds:duration.trim()?Math.max(0,Number(duration))*60:null,contact_name:contact||null,phone_used:client.mobile||client.phone1||null,appointment_created:showroom,purchase_amount:result==='COMPRO'?Number(purchaseAmount):null,next_action:showroom?'VALIDAR_SHOWROOM':nextAction||null,follow_up_date:followUp||null,notes:notes||null}
