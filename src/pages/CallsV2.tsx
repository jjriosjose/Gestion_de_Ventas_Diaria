...
import { CalendarClock,Check,History,PhoneCall,Search,ShoppingBag,Users } from 'lucide-react'
...
export function CallsV2(){
 ...
 return <div className="page-stack">
  ...
  {view==='PORTFOLIO'?<div className="split-grid" id="crm-cartera"><div className="panel"><div className="panel-head"><div><b>Cartera priorizada</b><span>Máximo 250 clientes visibles.</span></div><span className="badge">{filteredPortfolio.length}</span></div><div className="cards-list">{loading?<div className="skeleton tall"/>:filteredPortfolio.slice(0,250).map(c=>{const call=lastCallByClient.get(c.id),appt=latestAppointmentByClient.get(c.id);return <button key={c.id} className={`activity-card ${selectedClientId===c.id?'selected':''}`} onClick={()=>setSelectedClientId(c.id)}><div className="activity-main"><b>{c.legal_name}</b><span>{c.codempr} · {c.client_type||'SIN TIPO'} · {c.region||'—'} / {c.province||'—'} / {c.municipality||'—'}</span><small>V: {employeeName(c.vendor_employee_id)} · G: {employeeName(c.manager_employee_id)} · última llamada: {call?new Date(call.occurred_at).toLocaleString('es-DO'):'Nunca'}{appt?` · showroom ${String(appt.status).replaceAll('_',' ')}`:''}</small></div><span className="badge">Gestionar</span></button>})}</div></div><div>{selectedClient?<CallWorkbench client={selectedClient} employee={employee} lastCall={lastCallByClient.get(selectedClient.id)} lastVisit={lastVisitByClient.get(selectedClient.id)} lastAppointment={latestAppointmentByClient.get(selectedClient.id)} onSaved={load}/>:<div className="panel empty-state"><PhoneCall/><b>Selecciona un cliente para gestionar</b><span>El contexto de llamadas, visitas y showroom permanece en esta misma pantalla.</span></div>}</div></div>:<div className="panel"><div className="panel-head"><div><b>Historial de llamadas</b><span>{visibleCalls.length} registros visibles.</span></div></div><div className="cards-list">{visibleCalls.slice(0,500).map(r=><div className="activity-card" key={r.id}><div className="activity-icon"><PhoneCall/></div><div className="activity-main"><b>{r.clients?.legal_name||r.prospects?.legal_name||'Llamada'}</b><span>{new Date(r.occurred_at).toLocaleString('es-DO')} · {r.call_direction==='ENTRANTE'?'Entrante':'Saliente'} · {r.caller?.full_name||'—'}</span><small>{labelResult(r.result)}{r.purchase_amount?` · ${money(r.purchase_amount)}`:''}{r.next_action?` · ${labelNextAction(r.next_action)}`:''}</small></div></div>)}</div></div>}
 </div>
}
