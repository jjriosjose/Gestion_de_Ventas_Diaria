...
export function Calls() {
  ...
  const load = async () => {
    ...
    try {
      const [callRes, visitRes, appointmentRes, staffRes, portfolio] = await Promise.all([
        supabase.from('calls').select('*,clients(id,codempr,legal_name,vendor_employee_id,manager_employee_id),prospects(prospect_code,legal_name),caller:employees!calls_employee_id_fkey(full_name,employee_type)').order('occurred_at', { ascending: false }).limit(3000),
        supabase.from('visits').select('id,client_id,employee_id,started_at,ended_at,received,purchase_result,result,no_purchase_reason,contact_name,next_action,follow_up_date,notes,operator:employees!visits_employee_id_fkey(full_name)').not('ended_at', 'is', null).order('ended_at', { ascending: false }).limit(5000),
        supabase.from('appointments').select('id,client_id,status,appointment_at,requested_appointment_at,source_type,employee_id,assigned_manager_id,requested_by_employee_id,created_at').order('created_at', { ascending: false }).limit(3000),
  ...
  const lastCallByClient = useMemo(() => {
    const map = new Map<string, any>()
    rows.forEach(row => { if (row.client_id && !map.has(row.client_id)) map.set(row.client_id, row) })
    return map
  }, [rows])
  const lastVisitByClient = useMemo(() => {
    const map = new Map<string, any>()
    visits.forEach(row => { if (row.client_id && !map.has(row.client_id)) map.set(row.client_id, row) })
    return map
  }, [visits])
  const latestAppointmentByClient = useMemo(() => {
    const map = new Map<string, any>()
    appointments.forEach(row => { if (row.client_id && !map.has(row.client_id)) map.set(row.client_id, row) })
    return map
  }, [appointments])
  ...
  const basePortfolio = useMemo(() => clients.filter(c => {
    ...
    return true
  }), [clients, vendorFilter, managerFilter, clientFilter, q])
  ...
  useEffect(() => {
    if (clientFilter && !basePortfolio.some(c => c.id === clientFilter)) setClientFilter('')
  }, [basePortfolio, clientFilter])

  useEffect(() => {
    if (selectedClientId && !clients.some(c => c.id === selectedClientId)) setSelectedClientId('')
  ...
  const report = visibleCalls.map(r => ({
    Fecha: new Date(r.occurred_at).toLocaleString('es-DO'),
    EjecutadaPor: r.caller?.full_name || '',
  ...
  const chooseClient = (id: string) => {
    ...
    window.setTimeout(() => document.getElementById('crm-workbench')?.scrollIntoView({ behavior: 'smooth', block: 'start' }), 30)
  }

  const startFromFilters = () => {
    if (filteredPortfolio.length === 1) chooseClient(filteredPortfolio[0].id)
    else document.getElementById('crm-cartera')?.scrollIntoView({ behavior: 'smooth', block: 'start' })
  }

  return <div className="page-stack">
    <div className="page-head">
      <div><span className="eyebrow">CRM OPERATIVO</span><h2>Llamadas</h2><p>Cartera, contexto de visitas, showroom, seguimientos y gestión telefónica en una sola pantalla.</p></div>
      <div className="button-row"><button className="secondary" onClick={() => void exportXlsx('Gestion_Llamadas', report)}>Excel</button><button className="secondary" onClick={() => exportPdf('Gestión de Llamadas', report)}>PDF</button><button className="primary" onClick={startFromFilters}><PhoneCall size={18}/> Registrar llamada</button></div>
    </div>

    <div className="kpi-grid compact-kpis">
      <div className="kpi-card"><div className="kpi-icon"><PhoneCall/></div><div><span>Llamadas visibles</span><strong>{visibleCalls.length}</strong><small>{todayRows.length} hoy</small></div></div>
      ...
      <div className="kpi-card"><div className="kpi-icon"><ShoppingBag/></div><div><span>Compraron</span><strong>{purchaseCount}</strong><small>reportados en llamada</small></div></div>
    </div>

    <div className="panel planner-filter-panel">
      <div className="planner-filter-grid">
        <div className="search-field"><Search size={18}/><input value={q} onChange={e => setQ(e.target.value)} placeholder="Cliente, código, teléfono, contacto, provincia..."/></div>
        ...
        <select value={clientFilter} onChange={e => setClientFilter(e.target.value)}><option value="">Todos los clientes ({basePortfolio.length})</option>{basePortfolio.slice(0, 500).map(c => <option key={c.id} value={c.id}>{c.legal_name} · {c.codempr}</option>)}</select>
        <select value={crmFilter} onChange={e => setCrmFilter(e.target.value)}>{CRM_FILTERS.map(([value, label]) => <option value={value} key={value || 'all'}>{label}</option>)}</select>
        ...
        <select value={resultFilter} onChange={e => setResultFilter(e.target.value)}><option value="">Todos los resultados de llamada</option>{CALL_RESULTS.map(([value, label]) => <option value={value} key={value}>{label}</option>)}</select>
        <input type="date" value={from} onChange={e => setFrom(e.target.value)}/>
        <input type="date" value={to} onChange={e => setTo(e.target.value)}/>
      </div>
      <div className="planner-filter-actions"><div className="meta"><span>{filteredPortfolio.length} clientes CRM</span><span>{visibleCalls.length} llamadas</span></div><button className="secondary" onClick={() => { setQ(''); setVendorFilter(''); setManagerFilter(''); setClientFilter(''); setCallerFilter(''); setResultFilter(''); setCrmFilter(''); setFrom(''); setTo('') }}>Limpiar filtros</button></div>
    </div>

    {loadError && <div className="panel"><b>No fue posible cargar todos los datos del CRM.</b><span>{loadError}</span></div>}

    <div className="tabs">
      <button className={view === 'PORTFOLIO' ? 'active' : ''} onClick={() => setView('PORTFOLIO')}><Users size={16}/> Cartera / seguimiento</button>
      <button className={view === 'HISTORY' ? 'active' : ''} onClick={() => setView('HISTORY')}><History size={16}/> Historial de llamadas</button>
    </div>
    ...
    {view === 'PORTFOLIO' && <div id="crm-cartera" style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit,minmax(420px,1fr))', gap: 16, alignItems: 'start' }}>
      <div className="panel">
        <div className="panel-head"><div><b>Cartera priorizada</b><span>La lista conserva contexto de llamadas, visitas y showroom. Máximo 250 clientes visibles.</span></div><span className="badge">{filteredPortfolio.length} resultados</span></div>
        <div className="cards-list" style={{ maxHeight: 650, overflow: 'auto' }}>
          {!loading && filteredPortfolio.length === 0 && <div className="empty-state"><b>No hay clientes con estos filtros.</b></div>}
          {filteredPortfolio.slice(0, 250).map(client => {
            const call = lastCallByClient.get(client.id)
            const visit = lastVisitByClient.get(client.id)
            const appointment = latestAppointmentByClient.get(client.id)
            return <button key={client.id} className="activity-card" onClick={() => chooseClient(client.id)} style={{ width: '100%', textAlign: 'left', cursor: 'pointer', borderColor: selectedClientId === client.id ? 'var(--brand-red)' : undefined }}>
              <div className="activity-main">
                <b>{client.legal_name}</b>
                <span>{client.codempr} · V: {employeeName(client.vendor_employee_id)} · G: {employeeName(client.manager_employee_id)}</span>
                <small>Última llamada: {call ? `${new Date(call.occurred_at).toLocaleDateString('es-DO')} · ${labelResult(call.result)}` : 'Nunca'}</small>
                <small>Última visita: {visit ? `${new Date(visit.ended_at).toLocaleDateString('es-DO')} · ${visit.operator?.full_name || employeeName(visit.employee_id)} · ${visit.purchase_result || visit.result || 'Gestionada'}` : 'Nunca'}</small>
                {appointment && <small>Showroom: {appointment.status.replaceAll('_', ' ')}{appointment.appointment_at ? ` · ${new Date(appointment.appointment_at).toLocaleString('es-DO')}` : ''}</small>}
              </div>
              <div className="row-actions"><span className="badge">Gestionar</span></div>
            </button>
          })}
        </div>
      </div>

      <div id="crm-workbench" className="panel" style={{ position: 'sticky', top: 12 }}>
        {!selectedClient && <div className="empty-state"><PhoneCall size={28}/><b>Selecciona un cliente para gestionar</b><span>Filtra la cartera y pulsa un cliente. No tendrás que volver a buscarlo en otra ventana.</span></div>}
        {selectedClient && <ClientWorkbench key={selectedClient.id} employeeId={employee?.id || ''} client={selectedClient} employees={employees} lastCall={lastCallByClient.get(selectedClient.id)} lastVisit={lastVisitByClient.get(selectedClient.id)} appointment={latestAppointmentByClient.get(selectedClient.id)} onSaved={() => void load()}/>} 
      </div>
    </div>}

    {view === 'HISTORY' && <div className="cards-list">
      {visibleCalls.length === 0 && <div className="panel empty-state"><b>No hay llamadas con los filtros actuales.</b></div>}
      {visibleCalls.map(r => <div className="activity-card" key={r.id}><div className="activity-icon"><PhoneCall/></div><div className="activity-main"><b>{r.clients?.legal_name || r.prospects?.legal_name || 'Gestión telefónica'}</b><span>{r.caller?.full_name || 'Usuario'} · {new Date(r.occurred_at).toLocaleString('es-DO')}</span><small>V: {employeeName(r.clients?.vendor_employee_id)} · G: {employeeName(r.clients?.manager_employee_id)} · {labelResult(r.result)}{r.contact_name ? ` · ${r.contact_name}` : ''}</small>{r.next_action && <small>Próxima acción: {labelNextAction(r.next_action)}{r.follow_up_date ? ` · ${r.follow_up_date}` : ''}</small>}{r.notes && <small>{r.notes}</small>}</div>{r.appointment_created && <span className="badge success"><CalendarClock size={13}/> Showroom</span>}</div>)}
    </div>}
  </div>
}
...
function ClientWorkbench({ employeeId, client, employees, lastCall, lastVisit, appointment, onSaved }: { employeeId: string; client: any; employees: Employee[]; lastCall?: any; lastVisit?: any; appointment?: any; onSaved: () => void }) {
  ...
  useEffect(() => {
    ...
    else if (result === 'COMPRO') setNextAction('SEGUIMIENTO_PEDIDO')
    else if (result === 'LLAMAR_MAS_TARDE') setNextAction('LLAMAR_NUEVAMENTE')
  }, [result])
    ...
    try {
      ...
      if (wantsShowroom) {
        if (!client.manager_employee_id) {
          alert('Llamada guardada. El cliente no tiene V-Gestor asignado y no se pudo crear la solicitud showroom.')
        } else {
  ...
  return <div>
    <div className="panel-head"><div><span className="eyebrow">CLIENTE 360</span><b>{client.legal_name}</b><span>{client.codempr} · {client.phone1 || client.mobile || 'Sin teléfono'}</span></div><span className="badge">{employeeName(client.manager_employee_id)}</span></div>
    <div style={{ display: 'grid', gridTemplateColumns: 'repeat(2,minmax(0,1fr))', gap: 10, marginBottom: 14 }}>
      <div className="info-box"><PhoneCall size={17}/><div><b>Última llamada</b><span>{lastCall ? `${new Date(lastCall.occurred_at).toLocaleString('es-DO')} · ${labelResult(lastCall.result)}` : 'Nunca'}</span></div></div>
      <div className="info-box"><UserRoundCheck size={17}/><div><b>Última visita</b><span>{lastVisit ? `${new Date(lastVisit.ended_at).toLocaleString('es-DO')} · ${lastVisit.operator?.full_name || employeeName(lastVisit.employee_id)}` : 'Nunca'}</span></div></div>
      <div className="info-box"><ShoppingBag size={17}/><div><b>Resultado visita</b><span>{lastVisit?.purchase_result || lastVisit?.result || 'Sin gestión'}</span></div></div>
      <div className="info-box"><CalendarClock size={17}/><div><b>Showroom</b><span>{appointment ? appointment.status.replaceAll('_', ' ') : 'Sin cita'}</span></div></div>
    </div>
    {lastVisit?.notes && <div className="selected-client" style={{ marginBottom: 14 }}><div><b>Nota de la última visita</b><span>{lastVisit.notes}</span></div></div>}
    <div className="form-grid">
      ...
      {result === 'INTERESADO_SHOWROOM' && <div className="span-2 info-box"><CalendarClock size={18}/><div><b>Validación por V-Gestor</b><span>La solicitud quedará pendiente para {employeeName(client.manager_employee_id)}. No contará como cita pactada hasta confirmarse.</span></div></div>}
    ...
    <div className="modal-actions"><button className="primary" disabled={busy} onClick={() => void save()}>{busy ? 'Guardando...' : 'Guardar gestión CRM'}</button></div>
  </div>
}
...
async function loadPortfolio() {
  ...
  for (let from = 0; from < 5000; from += pageSize) {
    const { data, error } = await supabase.from('clients').select('id,codempr,legal_name,phone1,mobile,contact_name,vendor_employee_id,manager_employee_id,region,province,municipality,address1').order('legal_name').range(from, from + pageSize - 1)
    if (error) throw error
    all.push(...(data || []))
