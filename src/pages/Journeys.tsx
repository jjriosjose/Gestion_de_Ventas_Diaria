...
import { AlertTriangle,CalendarDays,CheckCircle2,ChevronRight,Clock3,Download,FileSpreadsheet,FilterX,Gauge,MapPin,Route,ShieldAlert,TimerReset,Users } from 'lucide-react'
...
const statusLabel:Record<string,string>={NO_INICIADA:'No iniciada',PLANIFICADA:'Planificada',PROGRAMADA:'Programada',FINALIZADA_PARCIAL:'Finalizada parcial',FINALIZADA:'Finalizada',PENDIENTE_CIERRE:'Pendiente de cierre',ACTIVA:'Activa hoy'}
...
export function Journeys(){
  ...
  const load=async()=>{
    ...
    let q=supabase.from('executive_route_journeys').select('*').gte('route_date',range.from).lte('route_date',range.to).order('route_date',{ascending:false}).order('started_at',{ascending:false})
  ...
  const regionOptions=useMemo(()=>unique(preTerritory.flatMap(r=>r.regions||[])),[preTerritory])
  const provinceOptions=useMemo(()=>unique(preTerritory.filter(r=>!region||r.regions?.includes(region)).flatMap(r=>r.provinces||[])),[preTerritory,region])
  const municipalityOptions=useMemo(()=>unique(preTerritory.filter(r=>(!region||r.regions?.includes(region))&&(!province||r.provinces?.includes(province))).flatMap(r=>r.municipalities||[])),[preTerritory,region,province])
  const clientTypeOptions=useMemo(()=>unique(rows.flatMap(r=>r.client_types||[])),[rows])
  const filtered=useMemo(()=>preTerritory.filter(r=>(!region||r.regions?.includes(region))&&(!province||r.provinces?.includes(province))&&(!municipality||r.municipalities?.includes(municipality))),[preTerritory,region,province,municipality])
  ...
  const openDetail=async(row:JourneyRow)=>{
    ...
    const [s,i]=await Promise.all([
      supabase.from('route_stops').select('id,stop_order,status,reason_not_visited,exception_reason_code,clients(codempr,legal_name,client_type,region,province,municipality)').eq('route_plan_id',row.route_plan_id).order('stop_order'),
