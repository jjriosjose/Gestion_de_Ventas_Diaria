...
import { CalendarRange,Camera,FilterX,ImagePlus,MapPin,Navigation,Plus,Target,Trash2,Upload,X } from 'lucide-react'
...
type CaptureContext={region?:string|null;province?:string|null;municipality?:string|null;district_municipality?:string|null;capture_mode?:'TAREA'|'LIBRE';assignment_name?:string|null;within_assigned_area?:boolean|null}
...
export function Capture(){
  const {employee}=useAuth();const canCreate=hasPermission(employee,'capture.create');const canAssign=hasPermission(employee,'planning.manage');const canDeleteTask=Boolean(employee?.active&&['Administrador','Supervisor'].includes(employee.app_role||''))
  ...
  const [taskStart,setTaskStart]=useState(today());const [taskEnd,setTaskEnd]=useState(addDays(today(),3));const [target,setTarget]=useState(10);const [includeSaturday,setIncludeSaturday]=useState(false);const [savingTask,setSavingTask]=useState(false);const [deletingTaskId,setDeletingTaskId]=useState('');const [selectedTaskId,setSelectedTaskId]=useState('')
  ...
  const report=visibleProspects.map(r=>({Codigo:r.prospect_code||'',Prospecto:r.legal_name,Contacto:r.contact_name||'',Telefono:r.phone||r.mobile||'',Modo:r.capture_mode==='TAREA'?'Tarea':'Libre',Region:r.region||'',Provincia:r.province||'',Municipio:r.municipality||'',Distrito:r.district_municipality||'',DentroZona:r.within_assigned_area===true?'Sí':r.within_assigned_area===false?'No':'—',Estado:r.status,Captado:r.employees?.full_name||'',Fecha:new Date(r.captured_at).toLocaleString('es-DO')}))
  ...
  const deleteTask=async(task:CaptureTask)=>{
    if(!canDeleteTask)return
    const captured=taskCounts.get(task.id)||0
    if(captured>0)return alert(`No se puede eliminar esta tarea porque ya tiene ${captured} prospecto(s) captado(s). Conserva la tarea para mantener la trazabilidad.`)
    if(!['BORRADOR','PLANIFICADA'].includes(task.status||''))return alert('Solo se pueden eliminar tareas de captación que todavía no han sido iniciadas.')
    const label=task.official_area_name||task.title||'esta tarea de captación'
    if(!window.confirm(`¿Eliminar definitivamente "${label}" asignada a ${employeeName(task.employee_id)}? Esta acción solo está permitida si la tarea no tiene actividad.`))return
    setDeletingTaskId(task.id)
    try{
      const{error}=await supabase.rpc('delete_unstarted_route_plan',{p_plan_id:task.id})
      if(error)throw new Error(error.message)
      if(selectedTaskId===task.id){setSelectedTaskId('');setOfficialSelection(EMPTY_OFFICIAL_SELECTION);setOfficialArea(null)}
      await load()
      alert('Tarea de captación eliminada correctamente.')
    }catch(error){
      alert(error instanceof Error?error.message:'No se pudo eliminar la tarea de captación')
  ...
  return <div className="page-stack">
    ...
    <div className="panel"><div className="panel-head"><div><b>{canAssign?'Prospectos captados':'Mis prospectos captados'}</b><span>Historial reciente de captación en calle con ubicación GPS y vínculo de tarea.</span></div></div><div className="cards-list">{visibleProspects.map(r=>{const area=r.municipality||r.province||r.gps_area_name||'Ubicación no determinada';const mode=r.capture_mode==='TAREA'?'Tarea asignada':'Captación libre';const zoneState=r.capture_mode==='TAREA'?(r.within_assigned_area===false?' · FUERA DE ZONA':r.within_assigned_area===true?' · Dentro de zona':''):'';return <div className="activity-card" key={r.id}><div className="activity-icon"><MapPin/></div><div className="activity-main"><b>{r.legal_name}</b><span>{r.prospect_code||'Prospecto'} · {area}</span><small>{r.status} · {mode}{zoneState} · Captado por {r.employees?.full_name||'—'}</small></div><span className="badge">{new Date(r.captured_at).toLocaleDateString('es-DO')}</span></div>})}</div></div>
...
function NewProspect({employeeId,tasks,preferredTaskId,onClose,onSaved}:{employeeId:string;tasks:CaptureTask[];preferredTaskId?:string;onClose:()=>void;onSaved:()=>void}){
  ...
  const [name,setName]=useState('');const [contact,setContact]=useState('');const [phone,setPhone]=useState('');const [type,setType]=useState('');const [interest,setInterest]=useState('');const [notes,setNotes]=useState('');const [assignmentId,setAssignmentId]=useState(initialTask);const [geo,setGeo]=useState<any>(null);const [context,setContext]=useState<CaptureContext|null>(null);const [files,setFiles]=useState<File[]>([]);const [busy,setBusy]=useState(false);const [freeConfirmed,setFreeConfirmed]=useState(false);const input=useRef<HTMLInputElement|null>(null)
  const preview=async(position:any,taskId=assignmentId)=>{const{data,error}=await supabase.rpc('preview_capture_context',{p_latitude:position.latitude,p_longitude:position.longitude,p_capture_assignment_id:taskId||null});if(error)throw error;setContext((data||null) as CaptureContext|null);return(data||null) as CaptureContext|null}
  const getGps=async()=>{try{const position=await currentPosition();setGeo(position);await preview(position)}catch(e){alert(e instanceof Error?e.message:'No fue posible determinar la ubicación')}}
  useEffect(()=>{if(!geo)return;void preview(geo,assignmentId).catch(()=>setContext(null))},[assignmentId])
  const save=async()=>{if(!name.trim()||!employeeId)return alert('Nombre obligatorio');if(tasks.length>0&&!assignmentId)return alert('Selecciona la tarea activa a la que pertenece esta captación.');if(tasks.length===0&&!freeConfirmed)return alert('Confirma que deseas registrar una Captación libre.');setBusy(true);try{const p=geo||await currentPosition();const captureContext=await preview(p,assignmentId);if(assignmentId&&captureContext?.within_assigned_area===false&&!window.confirm(`El GPS actual está fuera de la zona asignada${captureContext.assignment_name?` (${captureContext.assignment_name})`:''}. ¿Deseas registrar la captación de todas formas?`)){setBusy(false);return}const code=`PRO-${Date.now().toString().slice(-8)}`;const{data:prospect,error}=await supabase.from('prospects').insert({prospect_code:code,legal_name:name.trim(),contact_name:contact||null,phone:phone||null,mobile:phone||null,client_type:type||null,business_interest:interest||null,latitude:p.latitude,longitude:p.longitude,gps_accuracy_m:p.accuracy,status:'NUEVO',captured_by_employee_id:employeeId,capture_assignment_id:assignmentId||null,notes:notes||null}).select().single();if(error)throw error;for(const f of files){const ext=f.name.split('.').pop()?.toLowerCase()||'jpg';const path=`prospects/${prospect.id}/${crypto.randomUUID()}.${ext}`;const{error:up}=await supabase.storage.from('karaka-photos').upload(path,f,{contentType:f.type||'image/jpeg'});if(up)throw up;await supabase.from('photos').insert({prospect_id:prospect.id,employee_id:employeeId,bucket_id:'karaka-photos',object_path:path,photo_type:'FACHADA',mime_type:f.type,size_bytes:f.size,latitude:p.latitude,longitude:p.longitude,taken_at:new Date().toISOString()})}onSaved()}catch(e){alert(e instanceof Error?e.message:'Error guardando prospecto')}finally{setBusy(false)}}
  const locationLabel=context?[context.municipality,context.province,context.region].filter(Boolean).join(' · '):''
  return <div className="modal-wrap"><button className="modal-backdrop" onClick={onClose}/><div className="modal large"><div className="modal-head"><div><span className="eyebrow">CAPTACIÓN EN CALLE</span><h3>Nuevo prospecto</h3></div><button className="icon-btn" onClick={onClose}><X/></button></div>{tasks.length>0?<div className="assignment-callout"><Target size={18}/><div><b>Tarea de captación activa</b><span>La captación debe asociarse a una tarea activa para que sume al objetivo. El GPS validará si estás dentro de la zona asignada.</span></div></div>:<div className="assignment-callout"><MapPin size={18}/><div><b>Captación libre</b><span>No tienes una tarea de captación activa hoy. La ubicación territorial se determinará automáticamente por GPS y no sumará a un objetivo asignado.</span></div><label className="checkbox"><input type="checkbox" checked={freeConfirmed} onChange={e=>setFreeConfirmed(e.target.checked)}/> Confirmo registrar como Captación libre</label></div>}<div className="form-grid">{tasks.length>0&&<label className="span-2">Tarea activa *<select value={assignmentId} onChange={e=>setAssignmentId(e.target.value)}><option value="">Seleccionar tarea...</option>{tasks.map(task=><option key={task.id} value={task.id}>{task.official_area_name||task.title} · {humanDate(task.route_date)}–{humanDate(task.period_end_date||task.route_date)}</option>)}</select></label>}<label>Razón social / nombre *<input value={name} onChange={e=>setName(e.target.value)}/></label><label>Contacto<input value={contact} onChange={e=>setContact(e.target.value)}/></label><label>Teléfono / WhatsApp<input value={phone} onChange={e=>setPhone(e.target.value)}/></label><label>Tipo de negocio<input value={type} onChange={e=>setType(e.target.value)}/></label><label className="span-2">Interés comercial<input value={interest} onChange={e=>setInterest(e.target.value)}/></label><label className="span-2">Observación<textarea value={notes} onChange={e=>setNotes(e.target.value)}/></label></div><div className="capture-tools"><button className={geo?'success-btn':'secondary'} onClick={()=>void getGps()}><MapPin size={18}/>{geo?`GPS ±${Math.round(geo.accuracy)} m`:'Capturar ubicación'}</button><input ref={input} hidden type="file" accept="image/*" capture="environment" multiple onChange={e=>setFiles(Array.from(e.target.files||[]))}/><button className="secondary" onClick={()=>input.current?.click()}><Camera size={18}/> Tomar / cargar fotos</button><span>{files.length} foto(s)</span></div>{context&&<div className={`assignment-callout ${context.within_assigned_area===false?'warning':''}`}><MapPin size={18}/><div><b>{locationLabel||'Ubicación territorial determinada por GPS'}</b><span>{context.district_municipality?`Distrito municipal: ${context.district_municipality} · `:''}{context.capture_mode==='TAREA'?(context.within_assigned_area===false?'Fuera de la zona asignada':context.within_assigned_area===true?'Dentro de la zona asignada':'Zona asignada sin validación geométrica'):'Captación libre'}</span></div></div>}{files.length>0&&<div className="file-strip">{files.map((f,i)=><div key={i}><ImagePlus/><span>{f.name}</span></div>)}</div>}<div className="modal-actions"><button className="secondary" onClick={onClose}>Cancelar</button><button className="primary" disabled={busy||(tasks.length>0&&!assignmentId)||(tasks.length===0&&!freeConfirmed)} onClick={()=>void save()}><Upload size={18}/>{busy?'Guardando...':tasks.length?'Guardar prospecto':'Guardar captación libre'}</button></div></div></div>
}
