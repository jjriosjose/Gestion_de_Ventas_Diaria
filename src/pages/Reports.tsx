...
export function Reports(){
  ...
  const load=async()=>{
    ...
    const [e,g,t]=await Promise.all([
      supabase.from('executive_daily_employee_summary').select('*').eq('day',date).order('operational_seconds',{ascending:false}),
      supabase.from('executive_daily_global_summary').select('*').eq('day',date).maybeSingle(),
      supabase.from('executive_activity_timeline').select('*').eq('day',date).order('started_at',{ascending:true}),
    ])
  ...
  return <div className="page-stack executive-report">
    <div className="page-head"><div><span className="eyebrow">DIRECCIÓN · CIERRE OPERATIVO</span><h2>Reporte Ejecutivo Diario</h2><p>Lectura gerencial de actividad, tiempos, clientes, conversiones, ventas y eventualidades. Los tiempos estimados se identifican por separado.</p></div><div className="executive-head-actions"><label className="inline-date">Fecha<input type="date" value={date} onChange={e=>setDate(e.target.value)}/></label><button className="secondary" disabled={!rows.length} onClick={()=>void exportXlsx(`Reporte_Ejecutivo_${date}`,executiveDetail)}><FileSpreadsheet size={17}/> Excel ejecutivo</button><button className="secondary" disabled={!rows.length} onClick={()=>exportPdf(`Reporte Ejecutivo Diario ${date}`,executiveDetail)}><Download size={17}/> PDF ejecutivo</button></div></div>

    {error&&<div className="panel"><b>No fue posible cargar el reporte ejecutivo.</b><span>{error}</span></div>}
    {loading?<div className="panel empty-state"><b>Calculando cierre operativo...</b></div>:<>
      <section className="executive-hero panel"><div className="executive-title"><div className="executive-mark"><BarChart3/></div><div><span>Resumen de la operación</span><strong>{new Date(`${date}T12:00:00`).toLocaleDateString('es-DO',{weekday:'long',day:'2-digit',month:'long',year:'numeric'})}</strong><small>{global?.active_employees||0} colaboradores con actividad registrada</small></div></div><div className="executive-primary"><span>Horas operativas registradas</span><b>{duration(global?.operational_seconds)}</b><small>Incluye tiempos exactos y estimados identificados</small></div></section>

      <div className="executive-kpis">
        <Metric icon={<Users/>} label="Colaboradores activos" value={global?.active_employees||0} note="con actividad en el día"/>
        <Metric icon={<MapPinCheck/>} label="Clientes visitados" value={`${global?.visited_clients||0}/${global?.planned_clients||0}`} note={`${global?.route_execution_pct||0}% ejecución vs plan`}/>
        <Metric icon={<PhoneCall/>} label="Gestiones telefónicas" value={global?.calls||0} note={`${global?.calls_contacted||0} contactadas · ${global?.call_contact_rate_pct||0}%`}/>
        <Metric icon={<CalendarDays/>} label="Atenciones showroom" value={global?.showroom_attended||0} note={duration(global?.showroom_seconds)}/>
        <Metric icon={<ShoppingBag/>} label="Compras registradas" value={global?.purchase_clients||0} note={`${global?.visit_purchase_clients||0} calle · ${global?.showroom_purchase_clients||0} showroom`}/>
        <Metric icon={<ShoppingBag/>} label="Ventas registradas" value={money(global?.sales_amount)} note="monto capturado en la gestión"/>
        <Metric icon={<Clock3/>} label="Traslado / espera estimado" value={duration(global?.transit_wait_estimated_seconds)} note="ventana de ruta menos atención e incidencias"/>
        <Metric icon={<AlertTriangle/>} label="Eventualidades" value={global?.incidents||0} note={duration(global?.incident_seconds)}/>
      </div>

      <section className="panel executive-table-panel"><div className="panel-head"><div><b>Resumen por colaborador</b><span>Selecciona una persona para abrir el detalle cronológico del día.</span></div><span className="badge">{rows.length} con actividad</span></div><div className="responsive-table"><table className="executive-table"><thead><tr><th>Colaborador</th><th>Ventana</th><th>H. operativas</th><th>Llamadas</th><th>T. llamadas*</th><th>Visitas</th><th>T. clientes</th><th>Showroom</th><th>T. showroom</th><th>Trayecto/espera*</th><th>Event.</th><th>Compras</th><th>Ventas</th></tr></thead><tbody>{rows.map(r=><tr key={r.employee_id} className={selectedEmployee===r.employee_id?'selected-row':''} onClick={()=>setSelectedEmployee(r.employee_id)}><td data-label="Colaborador"><b>{r.full_name}</b><small>{r.job_title||r.employee_type}</small></td><td data-label="Ventana">{time(r.first_activity_at)}–{time(r.last_activity_at)}</td><td data-label="H. operativas"><b>{duration(r.operational_seconds)}</b><small>{r.registered_utilization_pct??0}% utilización registrada</small></td><td data-label="Llamadas">{r.calls}<small>{r.calls_contacted} contactadas</small></td><td data-label="T. llamadas*">{duration(r.call_estimated_seconds)}</td><td data-label="Visitas">{r.visited_clients}/{r.planned_clients}</td><td data-label="T. clientes">{duration(r.visit_seconds)}</td><td data-label="Showroom">{r.showroom_attended}</td><td data-label="T. showroom">{duration(r.showroom_seconds)}</td><td data-label="Trayecto/espera*">{duration(r.transit_wait_estimated_seconds)}</td><td data-label="Event.">{r.incidents}<small>{duration(r.incident_seconds)}</small></td><td data-label="Compras">{r.purchase_clients}<small>{r.visit_purchase_clients} calle · {r.showroom_purchase_clients} showroom</small></td><td data-label="Ventas"><b>{money(r.sales_amount)}</b></td></tr>)}</tbody></table></div><div className="executive-legend">* Tiempo estimado. Visitas, showroom y eventualidades usan marcas reales de inicio/fin. La ventana operativa va desde la primera hasta la última actividad identificada.</div></section>
...
function Metric({icon,label,value,note}:{icon:React.ReactNode;label:string;value:string|number;note:string}){return <div className="executive-metric"><div className="executive-metric-icon">{icon}</div><div><span>{label}</span><b>{value}</b><small>{note}</small></div></div>}
