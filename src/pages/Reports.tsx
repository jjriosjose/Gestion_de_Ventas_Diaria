...
export function Reports(){
  ...
  const load=async()=>{
    ...
    let routeMetricsQuery=supabase.from('executive_daily_route_metrics').select('*').eq('day',date)
    if(!executiveAccess&&employee?.id){
      employeeQuery=employeeQuery.eq('employee_id',employee.id)
      timelineQuery=timelineQuery.eq('employee_id',employee.id)
      routeMetricsQuery=routeMetricsQuery.eq('employee_id',employee.id)
    }
    ...
    const [e,g,t,rm]=await Promise.all([employeeQuery,globalPromise,timelineQuery,routeMetricsQuery])
    const err=e.error||g.error||t.error||rm.error
    if(err)setError(err.message)
    const metricMap=new Map((rm.data||[]).map((m:any)=>[m.employee_id,m]))
    const employeeRows=(e.data||[]).map((row:any)=>{
      const metric:any=metricMap.get(row.employee_id)||{}
      return {...row,estimated_distance_m:Number(metric.estimated_distance_m||0),start_to_first_m:Number(metric.start_to_first_m||0),between_visits_m:Number(metric.between_visits_m||0),last_to_end_m:Number(metric.last_to_end_m||0),gps_segments:Number(metric.gps_segments||0)}
    })
    const distanceTotal=employeeRows.reduce((sum:number,row:any)=>sum+Number(row.estimated_distance_m||0),0)
    const gpsSegments=employeeRows.reduce((sum:number,row:any)=>sum+Number(row.gps_segments||0),0)
    setRows(employeeRows)
    setGlobal(g.data?{...g.data,estimated_distance_m:distanceTotal,gps_segments:gpsSegments}:null)
    setTimeline(t.data||[])
    setSelectedEmployee(current=>current&&employeeRows.some((r:any)=>r.employee_id===current)?current:(employeeRows[0]?.employee_id||''))
    setLoading(false)
  }
  ...
  const executiveDetail=rows.map(r=>({Empleado:r.full_name,Cargo:r.job_title||'',Tipo:r.employee_type,PrimeraGestion:time(r.first_activity_at),UltimaGestion:time(r.last_activity_at),Ventana:duration(r.activity_window_seconds),'Jornada ruta':duration(r.route_window_seconds),'Horas operativas':duration(r.operational_seconds),Planificados:r.planned_clients,Visitados:r.visited_clients,'Cobertura real %':coveragePct(r),Resueltos:r.resolved_clients||0,'Resolución ruta %':resolutionPct(r),Recibidos:r.received_clients,'Atención clientes':duration(r.visit_seconds),'Promedio por visita':duration(avgVisitSeconds(r)),'Traslado/espera estimado':duration(r.transit_wait_estimated_seconds),'Distancia GPS estimada':distanceKm(r.estimated_distance_m),'Tramos GPS':r.gps_segments||0,Llamadas:r.calls,Contactados:r.calls_contacted,'Tiempo llamadas estimado':duration(r.call_estimated_seconds),Showroom:r.showroom_attended,'Tiempo showroom':duration(r.showroom_seconds),Eventualidades:r.incidents,'Tiempo eventualidades':duration(r.incident_seconds),ComprasVisita:r.visit_purchase_clients,ComprasShowroom:r.showroom_purchase_clients,Compras:r.purchase_clients,Ventas:money(r.sales_amount),Captaciones:r.prospects_captured,'Utilizacion registrada %':r.registered_utilization_pct??'','Contacto %':r.call_contact_rate_pct??''}))
  ...
  return <div className="page-stack executive-report executive-shell-v062">
    ...
    {loading?<div className="panel empty-state"><b>Calculando cierre operativo...</b></div>:<>
      ...
      <div className="executive-meter-grid">
        ...
        <Meter icon={<MapPinCheck/>} label="Cobertura real" value={`${summary?.visited_clients||0}/${summary?.planned_clients||0}`} note={`${summary?.route_execution_pct||0}% visitados vs plan`} progress={summary?.route_execution_pct}/>
        <Meter icon={<Route/>} label="Distancia GPS estimada" value={distanceKm(summary?.estimated_distance_m)} note={`${summary?.gps_segments||0} tramos GPS · no es odómetro`}/>
        ...
        <Meter icon={<Clock3/>} label="Traslado / espera" value={duration(summary?.transit_wait_estimated_seconds)} note="estimado, no equivale a conducción pura"/>
      ...
      {selected&&<section className="panel employee-executive-detail"><div className="panel-head"><div><span className="eyebrow">DETALLE DEL DÍA</span><b>{selected.full_name}</b><span>{selected.job_title||selected.employee_type} · {time(selected.first_activity_at)} → {time(selected.last_activity_at)}</span></div><div className="button-row"><span className="report-role-badge">{selected.employee_type}</span><button className="secondary compact" onClick={()=>void exportXlsx(`Cronologia_${selected.full_name}_${date}`,timelineExport)}>Excel detalle</button><button className="secondary compact" onClick={()=>exportPdf(`Cronología ${selected.full_name} ${date}`,timelineExport)}>PDF detalle</button></div></div>
        {selected.employee_type==='Vendedor'?<div className="employee-time-strip"><TimeBlock label="Jornada de ruta" value={selected.route_window_seconds}/><TimeBlock label="Atención clientes" value={selected.visit_seconds}/><TimeBlock label="Promedio / visita" value={avgVisitSeconds(selected)}/><TimeBlock label="Traslado / espera*" value={selected.transit_wait_estimated_seconds}/><ValueBlock label="Distancia GPS*" value={distanceKm(selected.estimated_distance_m)}/><TimeBlock label="Eventualidades" value={selected.incident_seconds}/><TimeBlock label="Tiempo operativo" value={selected.operational_seconds}/></div>:<div className="employee-time-strip"><TimeBlock label="Operativo" value={selected.operational_seconds}/><TimeBlock label="Llamadas*" value={selected.call_estimated_seconds}/><TimeBlock label="Showroom" value={selected.showroom_seconds}/><TimeBlock label="Ventana gestión" value={selected.activity_window_seconds}/><TimeBlock label="Eventualidades" value={selected.incident_seconds}/><TimeBlock label="Tiempo clientes" value={selected.visit_seconds}/></div>}
        <div className="employee-result-strip">{selected.employee_type==='Vendedor'&&<><span><b>{coveragePct(selected)}%</b> cobertura real</span><span><b>{selected.resolved_clients||0}/{selected.planned_clients||0}</b> resueltos · {resolutionPct(selected)}%</span><span><b>{selected.gps_segments||0}</b> tramos GPS</span></>}<span><b>{selected.calls}</b> intentos de llamada</span><span><b>{selected.calls_contacted}</b> contactados</span><span><b>{selected.visited_clients}</b> visitas</span><span><b>{selected.showroom_attended}</b> showroom</span><span><b>{selected.purchase_clients}</b> compras</span><span><b>{money(selected.sales_amount)}</b> ventas</span></div>
        {selected.employee_type==='Vendedor'&&<div className="route-manager-note"><b>* Distancia GPS estimada:</b> suma geodésica entre puntos de salida, visitas y cierre disponibles. No representa odómetro ni recorrido vial exacto.</div>}
...
function VendorSection({rows,selectedEmployee,onSelect}:{rows:any[];selectedEmployee:string;onSelect:(id:string)=>void}){const totals=teamTotals(rows);const totalCoverage=ratioPct(totals.visited,totals.planned);return <section className="panel report-team-card"><TeamHeader icon={<Route/>} title="Vendedores · Operación de calle" subtitle="Cobertura real, resolución, jornada, atención, distancia y resultado comercial" count={rows.length}/><div className="report-team-summary"><SummaryBox label="Planificados" value={totals.planned}/><SummaryBox label="Visitados" value={totals.visited}/><SummaryBox label="Cobertura real" value={`${totalCoverage}%`}/><SummaryBox label="Jornada acumulada" value={duration(totals.routeWindow)}/><SummaryBox label="Distancia GPS" value={distanceKm(totals.distance)}/><SummaryBox label="Ventas" value={money(totals.sales)}/></div>{rows.length?<div className="responsive-table"><table className="report-team-table"><thead><tr><th>Vendedor</th><th>Horario</th><th>Jornada</th><th>Plan</th><th>Visitados</th><th>Cobertura real</th><th>Resueltos</th><th>Resolución</th><th>Atención</th><th>Prom./visita</th><th>Traslado/espera*</th><th>Dist. GPS*</th><th>Compras</th><th>Ventas</th></tr></thead><tbody>{rows.map(r=><tr key={r.employee_id} className={selectedEmployee===r.employee_id?'selected-row':''} onClick={()=>onSelect(r.employee_id)}><td><b>{r.full_name}</b><small>{r.job_title||'Vendedor'}</small></td><td>{time(r.first_activity_at)}–{time(r.last_activity_at)}</td><td><b>{duration(r.route_window_seconds)}</b><small>{r.incidents||0} eventualidades</small></td><td>{r.planned_clients||0}</td><td><b>{r.visited_clients||0}</b><small>{r.received_clients||0} recibidos</small></td><td><b>{coveragePct(r)}%</b><small>visitados ÷ plan</small></td><td>{r.resolved_clients||0}</td><td><b>{resolutionPct(r)}%</b><small>paradas resueltas ÷ plan</small></td><td>{duration(r.visit_seconds)}</td><td>{duration(avgVisitSeconds(r))}</td><td>{duration(r.transit_wait_estimated_seconds)}</td><td><b>{distanceKm(r.estimated_distance_m)}</b><small>{r.gps_segments||0} tramos</small></td><td>{r.purchase_clients||0}</td><td><b>{money(r.sales_amount)}</b></td></tr>)}</tbody></table></div>:<div className="report-empty-role">No hay vendedores con actividad registrada en esta fecha.</div>}<div className="route-manager-note"><b>* Distancia GPS estimada:</b> distancia geodésica entre puntos operativos disponibles; no equivale al kilometraje real del vehículo.</div></section>}
...
function ValueBlock({label,value}:{label:string;value:string|number}){return <div><span>{label}</span><b>{value}</b></div>}
function teamTotals(rows:any[]){return rows.reduce((a,r)=>({planned:a.planned+Number(r.planned_clients||0),visited:a.visited+Number(r.visited_clients||0),resolved:a.resolved+Number(r.resolved_clients||0),routeWindow:a.routeWindow+Number(r.route_window_seconds||0),visitSeconds:a.visitSeconds+Number(r.visit_seconds||0),transit:a.transit+Number(r.transit_wait_estimated_seconds||0),distance:a.distance+Number(r.estimated_distance_m||0),gpsSegments:a.gpsSegments+Number(r.gps_segments||0),calls:a.calls+Number(r.calls||0),contacted:a.contacted+Number(r.calls_contacted||0),showroom:a.showroom+Number(r.showroom_attended||0),purchases:a.purchases+Number(r.purchase_clients||0),sales:a.sales+Number(r.sales_amount||0)}),{planned:0,visited:0,resolved:0,routeWindow:0,visitSeconds:0,transit:0,distance:0,gpsSegments:0,calls:0,contacted:0,showroom:0,purchases:0,sales:0})}
