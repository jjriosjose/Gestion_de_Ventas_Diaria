...
const zero={active_employees:0,planned_clients:0,visited_clients:0,received_clients:0,purchase_clients:0,sales_amount:0,calls:0,calls_contacted:0,appointments:0,showroom_attended:0,prospects_captured:0,routes_started:0,routes_completed:0,route_execution_pct:0,call_contact_rate_pct:0,estimated_distance_m:0,gps_segments:0}
...
export function Dashboard(){
 ...
 const load=async()=>{
  setBusy(true)
  const day=today()
  const [{data:g},{data:e},{data:rm},{count:clients},{count:geo},{count:verified}]=await Promise.all([
  ...
  const metricMap=new Map((rm||[]).map((m:any)=>[m.employee_id,m]))
  const employeeRows=(e||[]).map((row:any)=>{const metric:any=metricMap.get(row.employee_id)||{};return {...row,estimated_distance_m:Number(metric.estimated_distance_m||0),gps_segments:Number(metric.gps_segments||0),start_to_first_m:Number(metric.start_to_first_m||0),between_visits_m:Number(metric.between_visits_m||0),last_to_end_m:Number(metric.last_to_end_m||0)}})
  ...
  setGlobal({...zero,...(g||{}),estimated_distance_m:distanceTotal,gps_segments:gpsSegments})
 ...
 useEffect(()=>{
  ...
  const ch=tables.reduce((channel,table)=>channel.on('postgres_changes',{event:'*',schema:'public',table},()=>void load()),supabase.channel('dashboard-live-v064')).subscribe()
 ...
 const reportRows=employees.map(e=>({Empleado:e.full_name,Cargo:e.job_title||'',Tipo:e.employee_type,Planificados:e.planned_clients,Visitados:e.visited_clients,'Cobertura real %':coveragePct(e),Resueltos:e.resolved_clients||0,'Resolución ruta %':resolutionPct(e),'Jornada ruta segundos':e.route_window_seconds||0,'Atención clientes segundos':e.visit_seconds||0,'Traslado/espera estimado segundos':e.transit_wait_estimated_seconds||0,'Distancia GPS estimada km':Number(e.estimated_distance_m||0)/1000,'Tramos GPS':e.gps_segments||0,Recibidos:e.received_clients,Showroom:e.showroom_attended,Compraron:e.purchase_clients,Ventas:Number(e.sales_amount||0),Llamadas:e.calls,Contactados:e.calls_contacted,Citas:e.appointments,Captaciones:e.prospects_captured,RutasIniciadas:e.routes_started,RutasCerradas:e.routes_completed}))
 ...
 return <div className="page-stack executive-shell-v062">
  ...
  <div className="kpi-grid">
   ...
   <KpiCard label="Planificados" value={global.planned_clients||0} sub={`${global.route_execution_pct||0}% cobertura real`} Icon={Route}/>
   <KpiCard label="Visitados" value={global.visited_clients||0} sub={`${global.received_clients||0} recibidos`} Icon={MapPinCheck}/>
   <KpiCard label="Distancia GPS" value={distanceKm(global.estimated_distance_m)} sub={`${global.gps_segments||0} tramos · estimada`} Icon={Route}/>
  ...
  <div className="executive-section-grid">
   <TeamPanel title="Ranking de Vendedores" subtitle="Cobertura real, resolución, distancia y resultado comercial" icon={<Route size={19}/>} count={vendors.length}>{vendors.length?vendors.slice(0,8).map((e,i)=><VendorRow key={e.employee_id} row={e} position={i+1}/>):<Empty text="Sin actividad de vendedores para esta fecha."/>}</TeamPanel>
...
function VendorRow({row,position}:{row:any;position:number}){const coverage=coveragePct(row);const resolution=resolutionPct(row);return <div className="executive-rank-row"><span className="executive-rank-position">{position}</span><div className="executive-rank-person"><b>{row.full_name}</b><small>{row.job_title||'Vendedor'} · {coverage}% cobertura · {resolution}% resolución</small></div><div className="executive-rank-kpis"><span><b>{row.visited_clients||0}/{row.planned_clients||0}</b> visitas</span><span><b>{row.resolved_clients||0}</b> resueltos</span><span><b>{distanceKm(row.estimated_distance_m)}</b> GPS</span><span><b>{row.purchase_clients||0}</b> compras</span></div><div className="executive-rank-result"><strong>{money(row.sales_amount)}</strong><small>{row.routes_completed||0} rutas cerradas · {row.gps_segments||0} tramos GPS</small></div></div>}
