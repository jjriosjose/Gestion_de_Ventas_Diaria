...
import { estimateOperationalDistance } from './logisticsTripDistance'
...
const POINT_SOURCE:Record<string,string>={ORIGIN:'Origen',STOP_GPS:'GPS operativo',STOP_PLANNED:'Ubicación planificada',RETURN_GPS:'GPS retorno/cierre',RETURN_PLANNED:'Retorno planificado'}
...
export async function exportTripHistoryExcel({trips,stops,documents,filters}:ExportArgs){
  ...
  const estimatesByTrip=new Map<string,ReturnType<typeof estimateOperationalDistance>>()
  ;(trips as ExportTrip[]).forEach(trip=>{
    const ss=exportStops.filter(stop=>stop.trip_id===trip.id)
    const ee=[...(eventsByTrip.get(trip.id)||[])].sort((a,b)=>new Date(a.occurred_at).getTime()-new Date(b.occurred_at).getTime())
    estimatesByTrip.set(trip.id,estimateOperationalDistance(trip,ss,ee))
  })
  ...
  summary.mergeCells('A2:H2');summary.getCell('A2').value='La distancia operativa estimada suma segmentos rectos entre puntos disponibles del viaje, priorizando GPS operativo y usando ubicación planificada cuando falta GPS. No representa la ruta vial exacta.';summary.getCell('A2').font={color:{argb:'FF64748B'},italic:true,size:9};summary.getCell('A2').alignment={wrapText:true,vertical:'middle'};summary.getRow(2).height=34
  const totalLoaded=exportDocs.reduce((s,d)=>s+Number(d.packages_loaded||0),0),totalDelivered=exportDocs.reduce((s,d)=>s+Number(d.packages_delivered||0),0),totalReturned=exportDocs.reduce((s,d)=>s+Number(d.packages_returned||0),0),totalAmount=exportDocs.reduce((s,d)=>s+Number(d.amount||0),0),gpsCount=events.filter(e=>e.latitude!=null&&e.longitude!=null).length,coverage=events.length?gpsCount/events.length:0,totalEstimatedMeters=[...estimatesByTrip.values()].reduce((sum,item)=>sum+item.totalMeters,0)
  const meta:Array<[string,unknown,string,unknown,string,unknown,string,unknown]>=[
    ...
    ['Eventos',events.length,'Cobertura GPS',coverage,'Distancia estimada km',totalEstimatedMeters/1000,'Incidencias',incidents.length],
  ]
  ...
  ;(trips as ExportTrip[]).forEach(trip=>{
    ...
    const row=tripSheet.addRow({trip:trip.trip_code,date:asDate(trip.trip_date),status:TRIP_STATUS[trip.status]||trip.status,title:trip.title||'',driver:trip.driver_name_snapshot||'',phone:trip.driver_phone_snapshot||'',vehicle:trip.vehicle_plate_snapshot||'',vehicle_type:trip.vehicle_type_snapshot||'',carrier:trip.carrier_name_snapshot||'Operación propia',stops:ss.length||trip.total_stops,docs:dd.length||trip.total_documents,loaded:dd.reduce((s,d)=>s+Number(d.packages_loaded||0),0),delivered:dd.reduce((s,d)=>s+Number(d.packages_delivered||0),0),returned:dd.reduce((s,d)=>s+Number(d.packages_returned||0),0),amount:dd.reduce((s,d)=>s+Number(d.amount||0),0)||Number(trip.total_amount||0),departed:asDate(trip.departed_at),returned_at:asDate(trip.returned_at),completed:asDate(trip.completed_at),duration:minutes(trip.departed_at,trip.completed_at||trip.returned_at),events:ee.length,gps_events:gg.length,coverage:ee.length?gg.length/ee.length:0,estimated_km:estimate.totalMeters/1000,segments:estimate.segments.length,gps_stops:estimate.gpsStopPoints,planned_stops:estimate.plannedStopPoints,missing_stops:estimate.missingStops,gap,incidents:ii.length,complete:ss.filter(s=>s.status==='DELIVERED').length,partial:ss.filter(s=>s.status==='PARTIAL').length,exceptions:ss.filter(s=>['NOT_DELIVERED','RESCHEDULED','CANCELLED'].includes(s.status)).length,deviation:deviations.length?Math.max(...deviations):null,service:service.length?service.reduce((a,b)=>a+b,0)/service.length:null,notes:trip.notes||''});statusStyle(row.getCell('status'),trip.status)
  })
  tripSheet.getColumn('B').numFmt=DATE;['P','Q','R'].forEach(c=>tripSheet.getColumn(c).numFmt=DATETIME);tripSheet.getColumn('O').numFmt=MONEY;tripSheet.getColumn('V').numFmt='0.0%';tripSheet.getColumn('W').numFmt='0.000';finish(tripSheet)

  const segmentSheet=wb.addWorksheet('Tramos');segmentSheet.columns=[
    {header:'Viaje',key:'trip',width:25},{header:'Fecha',key:'date',width:13},{header:'# tramo',key:'segment',width:9},{header:'Desde',key:'from',width:30},{header:'Fuente desde',key:'from_source',width:20},{header:'Lat. desde',key:'from_lat',width:14},{header:'Lon. desde',key:'from_lon',width:14},{header:'Hasta',key:'to',width:30},{header:'Fuente hasta',key:'to_source',width:20},{header:'Lat. hasta',key:'to_lat',width:14},{header:'Lon. hasta',key:'to_lon',width:14},{header:'Distancia estimada m',key:'meters',width:20},{header:'Distancia estimada km',key:'km',width:20},{header:'Observación',key:'note',width:42},
  ]
  ;(trips as ExportTrip[]).forEach(trip=>{
    const estimate=estimatesByTrip.get(trip.id)!
    estimate.segments.forEach((segment,index)=>segmentSheet.addRow({trip:trip.trip_code,date:asDate(trip.trip_date),segment:index+1,from:segment.from.label,from_source:POINT_SOURCE[segment.from.source]||segment.from.source,from_lat:segment.from.latitude,from_lon:segment.from.longitude,to:segment.to.label,to_source:POINT_SOURCE[segment.to.source]||segment.to.source,to_lat:segment.to.latitude,to_lon:segment.to.longitude,meters:segment.distanceMeters,km:segment.distanceMeters/1000,note:'Segmento recto estimado entre puntos disponibles; no representa distancia vial exacta.'}))
  })
  segmentSheet.getColumn('B').numFmt=DATE;['F','G','J','K'].forEach(c=>segmentSheet.getColumn(c).numFmt='0.000000');segmentSheet.getColumn('L').numFmt='0';segmentSheet.getColumn('M').numFmt='0.000';finish(segmentSheet)
