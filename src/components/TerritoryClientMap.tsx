...
const DOMINICAN_REPUBLIC_HOME_ZOOM = 8.5
...
function resetHomeView(map: L.Map) {
  map.setView(DOMINICAN_REPUBLIC_CENTER, DOMINICAN_REPUBLIC_HOME_ZOOM, { animate: false })
}
...
export function TerritoryClientMap({ clients, geoAssessments = EMPTY_GEO_ASSESSMENTS, selectedIds = [], selectable = false, areaTools = false, zones = [], showZones = true, focusPoint = null, height = 520, onToggleClient, onAreaSelect }: Props) {
  ...
  const activeOfficialLayerRef = useRef<L.LayerGroup | null>(null)
  const draftLayerRef = useRef<L.LayerGroup | null>(null)
  const officialBoundaryLayerRef = useRef<L.TileLayer.WMS | null>(null)
  const lastActiveOfficialIdRef = useRef<string | null>(null)
  const [zoom, setZoom] = useState(DOMINICAN_REPUBLIC_HOME_ZOOM)
  ...
  const [officialBoundaryLoading, setOfficialBoundaryLoading] = useState(false)
  ...
  const selectedOrderMap = useMemo(() => new Map(selectedIds.map((id,index)=>[id,index+1])), [selectedIds])
  const geocoded = useMemo(() => clients.filter(isGeocoded), [clients])
  const activeOfficialZone = useMemo(() => zones.find((zone) => zone.id.startsWith('official-') && zone.geometry) || null, [zones])
  const savedZones = useMemo(() => zones.filter((zone) => !zone.id.startsWith('official-')), [zones])
  const savedZonesKey = useMemo(() => savedZones.map((zone) => `${zone.id}:${zone.geometry ? '1' : '0'}`).join('|'), [savedZones])
  ...
  useEffect(() => {
    ...
    const map = L.map(host.current, {
      ...
      zoomSnap: 0.25,
      zoomDelta: 0.5,
    }).setView(DOMINICAN_REPUBLIC_CENTER, DOMINICAN_REPUBLIC_HOME_ZOOM)
    ...
    activeOfficialLayerRef.current = L.layerGroup().addTo(map)
    ...
    return () => {
      ...
      activeOfficialLayerRef.current = null
    }
  }, [])
  ...
  useEffect(() => {
    ...
    if (!showOfficialBoundaries) {
      setOfficialBoundaryLoading(false)
      return
    }

    setOfficialBoundaryLoading(true)
    ...
    const finishLoading = () => setOfficialBoundaryLoading(false)
    layer.on('load', finishLoading)
    layer.on('tileerror', finishLoading)
    layer.addTo(map)
    officialBoundaryLayerRef.current = layer
    return () => {
      layer.off('load', finishLoading)
      layer.off('tileerror', finishLoading)
  ...
  useEffect(() => {
    ...
    const frame = window.requestAnimationFrame(() => {
      ...
      const createIndividual = (client: Client) => {
        ...
        const routeOrder = selectedOrderMap.get(client.id)
        ...
        const marker = L.circleMarker([client.latitude!, client.longitude!], { radius: selected ? 9 : 6, weight: selected ? 3 : 2, color: '#ffffff', fillColor, fillOpacity: 0.92 })
        ...
        const routeFlag = routeOrder ? `<strong>Parada ${String(routeOrder).padStart(2,'0')}</strong>` : ''

        marker.bindTooltip(`<div class="territorial-tooltip"><b>${escapeHtml(client.legal_name)}</b><span>${escapeHtml(client.codempr)}</span><span>${escapeHtml(client.municipality || client.province || 'Sin localidad')}</span>${routeFlag}${qualityFlag}${selected ? '<strong>✓ Seleccionado</strong>' : selectable ? '<strong>Pulse para seleccionar</strong>' : ''}</div>`, { direction: 'top', offset: [0, -6], opacity: 0.96 })
        marker.bindPopup(`<div class="map-popup"><b>${escapeHtml(client.legal_name)}</b><small>${escapeHtml(client.codempr)}</small>${routeOrder?`<span><b>Parada:</b> ${routeOrder}</span>`:''}<span><b>Maestro:</b> ${escapeHtml(masterTerritory)}</span><span><b>Coordenada:</b> ${escapeHtml(detectedTerritory)}</span><span><b>Calidad:</b> ${escapeHtml(quality)}</span><span>V: ${escapeHtml(client.v_cartera || '—')}</span><span>G: ${escapeHtml(client.g_cartera || '—')}</span><a target="_blank" rel="noreferrer" href="${navigation}">Navegar con Google Maps</a></div>`)
        ...
        if(routeOrder){
          L.marker([client.latitude!,client.longitude!],{
            ...
            zIndexOffset:1400,
            icon:L.divIcon({
              className:'planning-route-order-marker',
              html:`<span>${routeOrder}</span>`,
              iconSize:[24,24],
              iconAnchor:[12,12],
            }),
          }).addTo(layer)
      ...
      if (geocoded.length > 90 && zoom <= 12) {
        const selectedClients = geocoded.filter((client)=>selectedSet.has(client.id))
        const unselectedClients = geocoded.filter((client)=>!selectedSet.has(client.id))
        selectedClients.forEach(createIndividual)

        const cellPx = zoom <= 8 ? 95 : zoom <= 10 ? 78 : 62
        const buckets = new Map<string, Client[]>()
        unselectedClients.forEach((client) => {
        ...
        buckets.forEach((bucket) => {
          ...
          const clusterFill = mismatchCount ? '#d97706' : '#c71f2d'
          ...
          cluster.bindTooltip(`<div class="territorial-tooltip cluster"><b>${bucket.length} clientes</b><span>Pulse para acercar</span>${mismatchCount ? `<strong>⚠ ${mismatchCount} con diferencia territorial</strong>` : ''}</div>`, { direction: 'top', opacity: 0.96 })
  ...
  }, [geocoded, geoAssessments, mode, onToggleClient, selectable, selectedOrderMap, selectedSet, zoom])
  ...
  useEffect(() => {
    ...
    savedZones.forEach((zone) => {
      const geometry = normalizeGeoJson(zone.geometry)
      if (!geometry) return
      try {
        const feature = L.geoJSON(geometry as any, { style: { color: '#7c3aed', weight: 2, fillColor: '#7c3aed', fillOpacity: 0.08, dashArray: '6 5' } })
        feature.bindTooltip(`<b>${escapeHtml(zone.name)}</b><br><span>${escapeHtml(zone.territory_type || 'Zona')}</span>`)
        feature.addTo(layer)
      } catch (error) {
        console.warn('No fue posible dibujar una zona guardada', zone.id, error)
      }
    })
  }, [showZones, savedZonesKey, savedZones])

  useEffect(() => {
    const map = mapRef.current
    const layer = activeOfficialLayerRef.current
    if (!map || !layer) return
    layer.clearLayers()

    const activeId = activeOfficialZone?.id || null
    const geometry = normalizeGeoJson(activeOfficialZone?.geometry)

    if (!activeOfficialZone || !geometry) {
      if (lastActiveOfficialIdRef.current) {
        lastActiveOfficialIdRef.current = null
        const frame = window.requestAnimationFrame(() => {
          map.invalidateSize({ animate: false })
          resetHomeView(map)
        })
    ...
    try {
      ...
      feature.bindTooltip(`<b>${escapeHtml(activeOfficialZone.name)}</b><br><span>${escapeHtml(activeOfficialZone.territory_type || 'División territorial oficial')}</span>`, { sticky: true })
      feature.addTo(layer)
      ...
      const frame = window.requestAnimationFrame(() => {
        ...
        if (bounds.isValid()) map.fitBounds(bounds, { padding: [38, 38], maxZoom: 13, animate: true })
      })
      lastActiveOfficialIdRef.current = activeId
      return () => window.cancelAnimationFrame(frame)
    } catch (error) {
      console.error('No fue posible dibujar la división territorial oficial', activeOfficialZone.id, error)
    }
  }, [activeOfficialZone?.id, activeOfficialZone?.geometry])
  ...
  return <div className="territorial-map-shell" style={{ minHeight: height }}>
    ...
    <div className="territorial-map-view-control">
      ...
      <label className="official-boundary-toggle" title="Mostrar límites provinciales oficiales publicados por IDERD / IGN-JJHM"><input type="checkbox" checked={showOfficialBoundaries} onChange={(event) => setShowOfficialBoundaries(event.target.checked)}/><MapPinned size={14}/><span>{officialBoundaryLoading ? 'Cargando límites…' : 'Límites'}</span></label>
    </div>
    <div className="territorial-map-summary"><b>{geocoded.length.toLocaleString()} en mapa</b><span>{selectedIds.length.toLocaleString()} seleccionados</span>{mapUpdating && <span>Actualizando mapa…</span>}</div>
