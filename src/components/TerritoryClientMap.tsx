...
export function TerritoryClientMap({
  clients,
  selectedIds = [],
  selectable = false,
  areaTools = false,
  ...
  useEffect(() => {
    ...
    const createIndividual = (client: Client) => {
      ...
      const marker = L.circleMarker([client.latitude!, client.longitude!], {
        ...
        weight: selected ? 3 : 2,
        color: '#ffffff',
        fillColor: selected ? '#17865c' : '#c71f2d',
        fillOpacity: 0.92,
      })
      marker.bindTooltip(
        `<div class="territorial-tooltip"><b>${escapeHtml(client.legal_name)}</b><span>${escapeHtml(client.codempr)}</span><span>${escapeHtml(client.municipality || client.province || 'Sin localidad')}</span>${selected ? '<strong>✓ Seleccionado</strong>' : selectable ? '<strong>Pulse para seleccionar</strong>' : ''}</div>`,
    ...
    if (geocoded.length > 90 && zoom <= 12) {
      ...
      buckets.forEach((bucket) => {
        ...
        const cluster = L.circleMarker([latitude, longitude], {
          radius: Math.min(24, 11 + Math.log2(bucket.length) * 2.4),
          weight: 3,
          color: '#ffffff',
          fillColor: selectedCount ? '#17865c' : '#c71f2d',
          fillOpacity: 0.94,
        })
        cluster.bindTooltip(
          `<div class="territorial-tooltip cluster"><b>${bucket.length} clientes</b>${selectedCount ? `<span>${selectedCount} seleccionados</span>` : '<span>Pulse para acercar</span>'}</div>`,
          { direction: 'top', opacity: 0.96 },
        )
        cluster.bindPermanentTooltip(String(bucket.length), {
          direction: 'center',
      ...
      geocoded.forEach(createIndividual)
    }
  ...
  useEffect(() => {
    ...
    zones.forEach((zone) => {
      ...
      try {
        const feature = L.geoJSON(zone.geometry as any, {
          style: {
            color: '#7c3aed',
            weight: 2,
            fillColor: '#7c3aed',
        ...
        // Una geometría inválida no debe impedir cargar el mapa operativo.
      }
    })
  }, [showZones, zones])
  ...
  const applyPolygon = () => {
    ...
    const ids = geocoded
      .filter((client) => pointInPolygon(client.latitude!, client.longitude!, polygon))
      .map((client) => client.id)
    onAreaSelect?.(ids, { kind: 'POLYGON', points: polygon })
  }

  const applyRadius = () => {
    if (!radiusCenter) return
    const center = { latitude: radiusCenter[0], longitude: radiusCenter[1] }
    const ids = geocoded
      .filter((client) => haversineKm(center, client) <= radiusKm)
      .map((client) => client.id)
    onAreaSelect?.(ids, { kind: 'RADIUS', center: radiusCenter, radiusKm })
  }

  return (
    <div className="territorial-map-shell" style={{ minHeight: height }}>
      <div ref={host} className="territorial-map" style={{ minHeight: height }} />
      <div className="territorial-map-summary">
        <b>{geocoded.length.toLocaleString()} en mapa</b>
        <span>{selectedIds.length.toLocaleString()} seleccionados</span>
      </div>

      {areaTools && (
        <div className="territorial-map-tools">
          <button
          ...
          >
            <Pentagon size={16} /> Polígono
          </button>
          <button
            className={mode === 'RADIUS' ? 'active' : ''}
            onClick={() => {
              setMode(mode === 'RADIUS' ? 'NONE' : 'RADIUS')
              clearDraft()
            }}
            title="Seleccionar por radio"
          >
          ...
          {mode === 'POLYGON' && (
            <>
              <span className="tool-hint">{polygon.length < 3 ? 'Marca al menos 3 puntos' : `${polygon.length} puntos`}</span>
              <button disabled={!polygon.length} onClick={() => setPolygon((points) => points.slice(0, -1))}><Undo2 size={15} /></button>
              <button disabled={polygon.length < 3} className="apply" onClick={applyPolygon}>Seleccionar área</button>
              <button onClick={stopDrawing}><X size={15} /></button>
            </>
          )}
          ...
          {mode === 'RADIUS' && (
            <>
              <select value={radiusKm} onChange={(event) => setRadiusKm(Number(event.target.value))}>
                {[2, 3, 5, 8, 10, 15, 20].map((km) => <option value={km} key={km}>{km} km</option>)}
              </select>
              <span className="tool-hint">{radiusCenter ? 'Centro definido' : 'Pulsa el centro en el mapa'}</span>
              <button disabled={!radiusCenter} className="apply" onClick={applyRadius}>Seleccionar radio</button>
              <button onClick={stopDrawing}><X size={15} /></button>
            </>
          )}

          {mode === 'NONE' && <button onClick={clearDraft} title="Limpiar herramienta"><RotateCcw size={15} /></button>}
        </div>
      )}
    </div>
  )
