...
type AlertItem={id:string;title:string;message:string;created_at:string;category:Category;severity:Severity;synthetic:boolean;href:string;storedId?:string;entityKey?:string}
...
function inferStored(item:any):Pick<AlertItem,'category'|'severity'|'href'>{
 ...
 if(entity.includes('APPOINT')||entity.includes('RECEPTION')||type.includes('SHOWROOM')||type.includes('CITA')||type.includes('CLIENTE_RECEPCION'))return{category:'AGENDA',severity:'ACTION',href:item.entity_id?`/agenda?ref=${item.entity_id}`:'/agenda'}
 if(entity.includes('CALL')||entity.includes('FOLLOW')||type.includes('CRM'))return{category:'CRM',severity:'ACTION',href:'/llamadas'}
...
function iconFor(item:AlertItem){
 ...
 if(item.category==='JORNADAS')return <MapPinned/>
 if(item.category==='CRM')return <PhoneCall/>
 if(item.category==='AGENDA')return <CalendarCheck2/>
 if(item.severity==='SUCCESS')return <CheckCircle2/>
 if(item.severity==='ACTION')return <AlertTriangle/>
 return <Info/>
}

function relative(value:string){
 const diff=Date.now()-new Date(value).getTime(),min=Math.round(Math.abs(diff)/60000)
 if(min<60)return diff>=0?`hace ${Math.max(1,min)} min`:`en ${Math.max(1,min)} min`
 const h=Math.round(min/60);if(h<24)return diff>=0?`hace ${h} h`:`en ${h} h`
 const d=Math.round(h/24);return diff>=0?`hace ${d} d`:`en ${d} d`
}
...
export function NotificationCenterBell({onOpen}:{onOpen?:()=>void}){
 ...
 const load=async()=>{
  if(!employee?.id){setAlerts([]);return}
  setLoading(true)
  const now=new Date(),soon=new Date(now);soon.setDate(soon.getDate()+3)

  let journey:any=hasPermission(employee,'journeys.view')
   ?supabase.from('executive_route_journeys_v2').select('route_plan_id,employee_id,full_name,route_date,derived_status,planned_clients,visited_clients,coverage_pct').eq('derived_status','PENDIENTE_CIERRE').order('route_date').limit(12)
   :null
  if(journey&&!executive)journey=journey.eq('employee_id',employee.id)

  let appointment:any=supabase.from('appointments')
   .select('id,employee_id,assigned_manager_id,status,appointment_at,requested_appointment_at,clients(legal_name),prospects(legal_name)')
   .in('status',['PENDIENTE_VALIDACION','CONFIRMADA','REPROGRAMADA'])
   .or(`appointment_at.lte.${soon.toISOString()},requested_appointment_at.lte.${soon.toISOString()}`)
   .order('created_at',{ascending:false}).limit(30)
  if(!executive)appointment=appointment.eq('assigned_manager_id',employee.id)

  let follow:any=hasPermission(employee,'journeys.view')
   ?supabase.from('executive_crm_followups_v1').select('id,employee_id,full_name,due_at,derived_status,subject_name,source_type').in('derived_status',['VENCIDO','HOY','PROXIMO']).order('due_at').limit(20)
   :null
  if(follow&&!executive)follow=follow.eq('employee_id',employee.id)

  const[stored,a,j,f]=await Promise.all([
   supabase.from('notifications').select('*').eq('employee_id',employee.id).eq('status','UNREAD').order('created_at',{ascending:false}).limit(40),
   appointment,
   journey||Promise.resolve({data:[],error:null}),
   follow||Promise.resolve({data:[],error:null}),
  ])

  const items:AlertItem[]=[]
  const storedEntityKeys=new Set<string>()

  ;(stored.data||[]).forEach((n:any)=>{
   const inferred=inferStored(n)
   const entityKey=n.entity_type&&n.entity_id?`${n.entity_type}:${n.entity_id}`:undefined
   if(entityKey)storedEntityKeys.add(entityKey)
   items.push({id:`stored-${n.id}`,storedId:n.id,title:n.title,message:n.message||'',created_at:n.created_at,synthetic:false,entityKey,...inferred})
  })

  ;(j.data||[]).forEach((x:any)=>{
   items.push({
    id:`journey-${x.route_plan_id}`,
    title:executive?`Jornada pendiente · ${x.full_name}`:'Jornada pendiente de cierre',
    message:`${new Date(`${x.route_date}T12:00:00`).toLocaleDateString('es-DO')} · ${x.visited_clients||0}/${x.planned_clients||0} visitados · ${Number(x.coverage_pct||0)}% cobertura`,
    created_at:`${x.route_date}T23:59:59-04:00`,synthetic:true,category:'JORNADAS',severity:'CRITICAL',href:`/jornadas?tab=street&journey=${x.route_plan_id}`,
   })
  })

  ;(f.data||[]).forEach((x:any)=>{
   items.push({id:`follow-${x.id}`,title:x.derived_status==='VENCIDO'?'Seguimiento vencido':x.derived_status==='HOY'?'Seguimiento para hoy':'Próximo seguimiento',message:`${x.subject_name} · ${x.full_name||'Sin responsable'} · ${x.source_type||'CRM'}`,created_at:x.due_at,synthetic:true,category:'CRM',severity:x.derived_status==='PROXIMO'?'INFO':'ACTION',href:`/llamadas?followup=${x.id}`})
  })

  ;(a.data||[])
   .filter((x:any)=>{
    if(storedEntityKeys.has(`APPOINTMENT:${x.id}`))return false
    if(x.status==='PENDIENTE_VALIDACION')return true
    const when=x.appointment_at||x.requested_appointment_at
    return Boolean(when&&new Date(when).getTime()>=now.getTime()&&new Date(when).getTime()<=soon.getTime())
   })
   .forEach((x:any)=>{
    const name=x.clients?.legal_name||x.prospects?.legal_name||'Cliente'
    const when=x.appointment_at||x.requested_appointment_at||now.toISOString()
    items.push({id:`appointment-${x.id}`,title:x.status==='PENDIENTE_VALIDACION'?'Showroom pendiente de validar':'Cita showroom próxima',message:`${name} · ${new Date(when).toLocaleString('es-DO')}`,created_at:when,synthetic:true,category:'AGENDA',severity:x.status==='PENDIENTE_VALIDACION'?'ACTION':'INFO',href:`/agenda?appointment=${x.id}`})
   })

  const unique=new Map<string,AlertItem>();items.forEach(i=>unique.set(i.id,i))
  const rank=(s:Severity)=>s==='CRITICAL'?0:s==='ACTION'?1:s==='INFO'?2:3
  setAlerts([...unique.values()].sort((a,b)=>rank(a.severity)-rank(b.severity)||new Date(a.created_at).getTime()-new Date(b.created_at).getTime()))
  setLoading(false)
 }
