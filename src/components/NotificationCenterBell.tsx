...
export function NotificationCenterBell({onOpen}:{onOpen?:()=>void}){
 const {employee}=useAuth();const navigate=useNavigate();const location=useLocation();const [open,setOpen]=useState(false),[alerts,setAlerts]=useState<AlertItem[]>([]),[filter,setFilter]=useState<'ALL'|'ACTION'>('ALL'),[loading,setLoading]=useState(false)
 ...
 const load=async()=>{
  ...
  let appointment:any=supabase.from('appointments').select('id,employee_id,assigned_manager_id,status,appointment_at,requested_appointment_at,clients(legal_name),prospects(legal_name)').in('status',['PENDIENTE_VALIDACION','CONFIRMADA','REPROGRAMADA']).or(`appointment_at.lte.${soon.toISOString()},requested_appointment_at.lte.${soon.toISOString()}`).order('created_at',{ascending:false}).limit(30)
  ...
  const[stored,a,j,f]=await Promise.all([supabase.from('notifications').select('*').eq('employee_id',employee.id).eq('status','UNREAD').order('created_at',{ascending:false}).limit(40),appointment,journey||Promise.resolve({data:[],error:null}),follow||Promise.resolve({data:[],error:null})])
  const items:AlertItem[]=[]
  const storedEntityKeys=new Set<string>()
  ;(stored.data||[]).forEach((n:any)=>{const inferred=inferStored(n),entityKey=n.entity_type&&n.entity_id?`${n.entity_type}:${n.entity_id}`:undefined;if(entityKey)storedEntityKeys.add(entityKey);items.push({id:`stored-${n.id}`,storedId:n.id,title:n.title,message:n.message||'',created_at:n.created_at,synthetic:false,entityKey,...inferred})})
 ...
 const visible=useMemo(()=>filter==='ALL'?alerts:alerts.filter(a=>a.severity==='CRITICAL'||a.severity==='ACTION'),[alerts,filter]),grouped=useMemo(()=>categoryOrder.map(category=>({category,items:visible.filter(i=>i.category===category)})).filter(g=>g.items.length),[visible])
