...
old_call = "{finish&&<FinishShowroom data={finish} employeeId={employee?.id||''} managers={managers} canEditAmount={amountAdmin} onClose={()=>setFinish(null)} onSaved={async()=>{setFinish(null);await load()}}/>}"
new_call = "{finish&&<FinishShowroom data={finish} employeeId={employee?.id||''} managers={managers} onClose={()=>setFinish(null)} onSaved={async()=>{setFinish(null);await load()}}/>}"
if old_call not in s:
    raise SystemExit('AgendaV2.tsx: FinishShowroom invocation not found')
s = s.replace(old_call, new_call, 1)

new_block = r'''function FinishShowroom({data,employeeId,managers,onClose,onSaved}:{data:any;employeeId:string;managers:any[];onClose:()=>void;onSaved:()=>Promise<void>|void}){
 ...
 const save=async()=>{
  ...
  if(purchased&&!amountMode)return alert('Indica si registrarás el monto ahora o quedará pendiente de registro.')
  if(purchased&&amountMode==='NOW'&&!(Number(amount)>0))return alert('Digita un monto de venta mayor que cero.')
  setBusy(true)
  try{
